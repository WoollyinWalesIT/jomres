...
jr_define( '_JOMRES_FRONT_IMAGEUPLOADS_INFO', 'Använd detta formulär för att ladda upp flera bilder av denna verksamhet. <br/>Observera att alla bilder med samma namn skrivs över och att detta formulär inte laddar upp inkvarteringsspecifika bilder eller verksamhetens huvudbild, dessa måste du ladda upp från Verksamhetsadministrationen.<br/>Observera att bilder som laddas upp här kommer att skrivas till /jomres/uploadedimages/*verksamhets_id*. Om mappen inte finns, kommer den att skapas, utom då en alternativ katalog har angetts i konfigurationen.' );
...
jr_define( '_JOMRES_A_GLOBALPFEATURES_INFO', 'Om du vill tilldela en bild till funktionen måste du först ladda upp dina bilder för verksamhetsegenskapen till mappen /jomres/uploadedimages/pfeatures.' );
...
jr_define( 'JOMRES_COM_A_MAPSKEY_DESC', 'Du kan få en Google Maps API nyckel från <a href="http://www.google.com/apis/maps/signup.html" target="_blank">Google maps</a>. När du har matat in din kartnyckel här kommer Jomres att visa kartan i verksamhetens informationssida.' );
...
if ( !defined( '_PN_OF' ) ) jr_define( '_PN_OF', 'av' );
if ( !defined( '_PN_START' ) ) jr_define( '_PN_START', 'Start' );
if ( !defined( '_PN_PREVIOUS' ) ) jr_define( '_PN_PREVIOUS', 'Föregående' );
if ( !defined( '_PN_NEXT' ) ) jr_define( '_PN_NEXT', 'Nästa' );
if ( !defined( '_PN_END' ) ) jr_define( '_PN_END', 'Slut' );
if ( !defined( '_PN_RESULTS' ) ) jr_define( '_PN_RESULTS', 'Resultat' );
...
jr_define( '_JOMRES_COM_SEF_NOTINSTALLED', 'Endast för Joomla 1.5. Antingen är she404sef inte installerad, eller så har du inte kopierat ' . JOMRESCONFIG_ABSOLUTE_PATH . JRDS . 'jomres' . JRDS . 'libraries' . JRDS . 'jomres' . JRDS . 'Cms_specific' . JRDS . 'joomla15' . JRDS . 'other_component_files' . JRDS . 'sh404sef' . JRDS . 'com_jomres.php till' . JOMRESCONFIG_ABSOLUTE_PATH . JRDS . 'components' . JRDS . 'com_sh404sef' . JRDS . 'sef_ext. När du har installerat sh404sef och/eller kopierat filen, då du kommer att kunna konfigurera SEF-URLer härifrån.<br/>Observera, om du även använder Joomfish v2 kan du behöva se till att GZIP är avstängd i globala konfigurationen.' );
...
jr_define( '_JOMRES_COM_A_CRON_METHOD_DESC', "Om du inte har tillgång till periodiska uppdrag, sätt detta på Plugin, skapa annars ett periodiskt uppdrag och kör den<br /> <i>curl -s" . JOMRES_SITEPAGE_URL_AJAX . "&task=cronjobs&tmpl=".get_showtime("tmplcomponent")."&no_html=1&secret=" . $jomresConfig_secret . "> /dev/null</i>" );
...
jr_define( '_JOMRES_COM_ALLOWHTMLEDITOR_DESC', 'Välj Ja för att aktivera HTML redigerare på sidan för verksamhetsredigering. Om valet är Nej tas alla html-taggar, utom <p> och <br />, bort från all inmatad och visad text.' );
...
jr_define( '_JOMRES_MANAGER_CHOOSE_SEARCH_INSTRUCTIONS', 'Skriv några bokstäver i användarnamnet för att söka efter en användare.<br/> När du väljer en användare läggs den automatiskt till som Administratör och nästa sida visas där du kan tilldela verksamheter och rättigheter för användaren.' );
