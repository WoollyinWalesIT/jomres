 ...
 *
 */(function (a) {
	var b, c = this, d = c.document, e = a(d), f = a(c), g = Array.prototype, h = 1.27, i = !0, j = 3e4, k = !1, l = navigator.userAgent.toLowerCase(), m = c.location.hash.replace(/#\//, ""), n = function () {
	}, o = function () {
		return!1
	}, p = function () {
		...
		if (!!a && a in this.trunk)b.call(x, a); else if (typeof a == "undefined")for (d in this.trunk)this.trunk.hasOwnProperty(d) && b.call(x, d)
		...
		return{array: function (a) {
			...
			return a("link[rel=stylesheet]").each(function () {
			...
			}), typeof f == "function" && (g = f, f = b), g = g || n, h ? (g.call(h, h), h) : (j = d.styleSheets.length, a("#" + f).length ? (a("#" + f).attr("href", e), j--, i = !0) : (h = a("<link>").attr({rel: "stylesheet", href: e, id: f}).get(0), c.setTimeout(function () {
				...
				if (p) {
					...
					f.href = e, !/file/.test(g.protocol) && g.hostname == f.hostname && g.port == f.port && g.protocol == f.protocol ? a.ajax({url: e, success: function () {
						i = !0
					}, error: k}) : k()
				}
			}, 10)), typeof g == "function" && E.wait({until: function () {
	...
	Galleria = function () {
		var g = this;
		this._options = {}, this._playing = !1, this._playtime = 5e3, this._active = null, this._queue = {length: 0}, this._data = [], this._dom = {}, this._thumbnails = [], this._layers = [], this._initialized = !1, this._firstrun = !1, this._stageWidth = 0, this._stageHeight = 0, this._target = b, this._id = parseInt(Math.random() * 1e4, 10);
		...
		var j = this._keyboard = {keys: {UP: 38, DOWN: 40, LEFT: 37, RIGHT: 39, RETURN: 13, ESCAPE: 27, BACKSPACE: 8, SPACE: 32}, map: {}, bound: !1, press: function (a) {
			var b = a.keyCode || a.which;
			b in j.map && typeof j.map[b] == "function" && j.map[b].call(g, a)
		}, attach: function (a) {
			var b, c;
			for (b in a)a.hasOwnProperty(b) && (c = b.toUpperCase(), c in j.keys ? j.map[j.keys[c]] = a[b] : j.map[c] = a[b]);
			j.bound || (j.bound = !0, e.bind("keydown", j.press))
		}, detach: function () {
			j.bound = !1, j.map = {}, e.unbind("keydown", j.press)
		}}, k = this._controls = {0: b, 1: b, active: 0, swap: function () {
			k.active = k.active ? 0 : 1
		}, getActive: function () {
			return k[k.active]
		}, getNext: function () {
			return k[1 - k.active]
		}}, l = this._carousel = {next: g.$("thumb-nav-right"), prev: g.$("thumb-nav-left"), width: 0, current: 0, max: 0, hooks: [], update: function () {
			var b = 0, c = 0, d = [0];
			...
			}), g.$("thumbnails").css({width: b, height: c}), l.max = b, l.hooks = d, l.width = g.$("thumbnails-list").width(), l.setClasses(), g.$("thumbnails-container").toggleClass("galleria-carousel", b > l.width), l.width = g.$("thumbnails-list").width()
			...
			if (isNaN(b))return;
			E.animate(g.get("thumbnails"), {left: b}, {duration: g._options.carouselSpeed, easing: g._options.easing, queue: !1})
		}}, m = this._tooltip = {initialized: !1, open: !1, timer: "tooltip" + g._id, swapTimer: "swap" + g._id, init: function () {
			m.initialized = !0;
			...
			!isNaN(e) && !isNaN(f) && (e += 10, f -= 30, e = Math.max(0, Math.min(k, e)), f = Math.max(0, Math.min(l, f)), c < j && (f = j), d.css({left: e, top: f}))
		}, bind: function (b, c) {
			...
			var d = function (b, c) {
				m.define(b, c), a(b).hover(function () {
					E.clearTimer(m.swapTimer), g.$("container").unbind("mousemove", m.move).bind("mousemove", m.move).trigger("mousemove"), m.show(b), E.addTimer(m.timer, function () {
						g.$("tooltip").stop().show().animate({opacity: 1}), m.open = !0
					}, m.open ? 0 : 500)
				},function () {
					g.$("container").unbind("mousemove", m.move), E.clearTimer(m.timer), g.$("tooltip").stop().animate({opacity: 0}, 200, function () {
		...
		}}, o = this._fullscreen = {scrolled: 0, crop: b, transition: b, active: !1, keymap: g._keyboard.map, os: {callback: n, support: function () {
			var a = q().html;
			...
			o.active = !0, E.hide(g.getActiveImage()), g.$("container").addClass("fullscreen"), o.scrolled = f.scrollTop(), E.forceStyles(g.get("container"), {position: "fixed", top: 0, left: 0, width: "100%", height: "100%", zIndex: 1e4});
			var d = {height: "100%", overflow: "hidden", margin: 0, padding: 0}, e = g.getData(), h = g._options;
			E.forceStyles(q().html, d), E.forceStyles(q().body, d), o.keymap = a.extend({}, g._keyboard.map), g.attachKeyboard({escape: g.exitFullscreen, right: g.next, left: g.prev}), o.crop = h.imageCrop, h.fullscreenCrop != b && (h.imageCrop = h.fullscreenCrop);
			if (e && e.big && e.image !== e.big) {
				var i = new Galleria.Picture, j = i.isCached(e.big), k = g.getIndex(), l = g._thumbnails[k];
				g.trigger({type: Galleria.LOADSTART, cached: j, rewind: !1, index: k, imageTarget: g.getActiveImage(), thumbTarget: l, galleriaData: e}), i.load(e.big, function (b) {
					g._scaleImage(b, {complete: function (b) {
						g.trigger({type: Galleria.LOADFINISH, cached: j, index: k, rewind: !1, imageTarget: b.image, thumbTarget: l});
		...
		o.os.listen();
		var r = this._idle = {timer: "idle" + g._id, trunk: [], bound: !1, add: function (b, c) {
			if (!b)return;
			r.bound || r.addEvent(), b = a(b);
			var d = {}, e;
			for (e in c)c.hasOwnProperty(e) && (d[e] = b.css(e));
			b.data("idle", {from: d, to: c, complete: !0, busy: !1}), r.addTimer(), r.trunk.push(b)
		}, remove: function (b) {
			...
			g.trigger(Galleria.IDLE_ENTER), a.each(r.trunk, function (a, b) {
				var c = b.data("idle");
				if (!c)return;
				b.data("idle").complete = !1, E.animate(b, c.to, {duration: g._options.idleSpeed})
			})
		}, showAll: function () {
			...
			}), s.image = new Galleria.Picture, a.each({box: "shadow content close prevholder nextholder", info: "title counter", content: "info image", prevholder: "prev", nextholder: "next"}, function (b, c) {
				var d = [];
			...
			b ? a(s.elems.box).css(j) : a(s.elems.box).animate(j, {duration: g._options.lightboxTransitionSpeed, easing: g._options.easing, complete: function () {
				var b = s.image, c = g._options.lightboxFadeSpeed;
				g.trigger({type: Galleria.LIGHTBOX_IMAGE, imageTarget: b.image}), a(b.container).show(), a(b.image).animate({opacity: 1}, c), E.show(s.elems.info, c)
			}})
		}, hide: function () {
			...
			s.active = b = typeof b == "number" ? b : g.getIndex() || 0, s.initialized || s.init(), s.keymap || (s.keymap = a.extend({}, g._keyboard.map), g.attachKeyboard({escape: s.hide, right: s.showNext, left: s.showPrev})), f.unbind("resize", s.rescale);
			...
			s.image.isIframe = !!d.iframe, a(s.elems.box).toggleClass("iframe", !!d.iframe), s.image.load(d.iframe || d.big || d.image, function (g) {
				s.width = g.isIframe ? a(c).width() : g.original.width, s.height = g.isIframe ? a(c).height() : g.original.height, a(g.image).css({width: g.isIframe ? "100%" : "100.1%", height: g.isIframe ? "100%" : "100.1%", top: 0, zIndex: 99998, opacity: 0, visibility: "visible"}), s.elems.title.innerHTML = d.title || "", s.elems.counter.innerHTML = b + 1 + " / " + e, f.resize(s.rescale), s.rescale()
		...
		a.extend(!0, e, Galleria.theme.defaults, this._original.options, Galleria.configure.options), function (a) {
			...
			var a = this.$("container"), d = {width: 0, height: 0}, e = function () {
		...
		return p === 8 && (b.parent().scrollTop(i * -1).scrollLeft(h * -1), b.css({top: 0, left: 0})), this.$("stage").unbind("mousemove", r).bind("mousemove", r), E.addTimer("pan" + c._id, s, 50, !0), this
	}, proxy: function (a, b) {
		...
		if (this._playing) {
			...
			var f = function () {
				...
				a._playing && (a.trigger({type: Galleria.PROGRESS, percent: Math.ceil(b / a._playtime * 100), seconds: Math.floor(b / 1e3), milliseconds: b}), E.addTimer(e, f, c))
			};
			E.addTimer(e, f, c)
		}
		...
		if (!b)return;
		return Galleria.on.binds.push({type: b, callback: c || n}), a.each(Galleria.get(), function (a, d) {
			d.bind(b, c)
		}), Galleria
