...
switch ( $field )
{
	...
	default:
		if ( $MiniComponents->eventSpecificlyExistsCheck( '05040', $field ) )
			...
			if ( !isset( $reply[ 'rebuild_rooms_list' ] ) ) $doNotRebuildRoomsListOnTheseFieldsArray[ ] = $field;
			if ( isset( $reply[ 'reply_to_echo' ] ) ) echo $reply[ 'reply_to_echo' ];
			}
		else
			{
...
if ( $field != "heartbeat" && $field != "show_log" && $field != "email_usage_check" )
	...
	if ( $bkg->checkArrivalDate( $arrivalDate ) )
		...
		if ( !in_array( $lastfield, $doNotRebuildRoomsListOnTheseFieldsArray ) )
			...
			if ( $bkg->getGuestVariantCount() > 0 ) echo '; populateDiv("totalinparty","' . $bkg->getTotalInParty() . '")';
			...
			if ( get_showtime( 'include_room_booking_functionality' ) )
				...
				if ( $bkg->cfg_tariffmode == "1" && $bkg->cfg_tariffChargesStoredWeeklyYesNo == "1" ) $room_per_night = $room_per_night * 7;
				$room_per_night = $bkg->get_rate_per_night_converted_to_output_period( $room_per_night );
				if ( get_showtime( 'include_room_booking_functionality' ) ) echo '; populateDiv("roompernight","' . output_price( $room_per_night ) . '")';
	...
	if ( $bkg->resetPricingOutput ) $bkg->outputZeroPrices();

	$disable_address = 'true';
	if ( $field == "email_usage_check" ) $disable_address = 'false';
	...
	if ( $bkg->getOkToBook() )
		...
		if ( $firstrun != "1" ) echo $bkg->setGuestPopupMessage( $bkg->monitorGetFirstMessage() );
...
function bookingformlistRooms( $isSingleRoomProperty, &$bkg )
	...
	if ( get_showtime( 'include_room_booking_functionality' ) )
		...
		if ( $isSingleRoomProperty ) $bkg->requestedRoom = array ();
		...
		if ( count( $freeRoomsArray ) > 0 ) $freeRoomsArray = $bkg->findFreeRoomsInDateRange( $freeRoomsArray );
		...
		foreach ( $selectedRoomUids as $room_uid_holder )
			...
			foreach ( $room_uid_holder as $key => $room_uid )
				...
				if ( is_array( $freeRoomsArray ) )
					{
					if ( !in_array( $key, $freeRoomsArray ) ) $bkg->removeFromSelectedRooms( $room_uid );
					}
				}
			}
		if ( $bkg->cfg_booking_form_rooms_list_style == "1" )
			{
			$bkg->setErrorLog( "handlereq-bookingformlistRooms:: Number of free rooms " . count( $freeRoomsArray ) );
			if ( count( $freeRoomsArray ) > 0 ) $freeRoomsArray = $bkg->removeRoomuidsAlreadyInThisBooking( $freeRoomsArray );
			}
