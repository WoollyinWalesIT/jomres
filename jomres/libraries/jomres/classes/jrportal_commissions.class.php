...
class jrportal_commissions // Functions supplied as a class so that they can be extended easily without rewriting  the functions
	...
	function deleteCrates( $id = false )
		{
		$clause = "";
	...
	function makeCrateDropdown( $selected = 1, $idx = null, $params = array() )
		{
		$options = array ();
		$js		 = "";
	...
	function makeCrateTypeDropdown($selected = 1)
		{
		$options = array ();
	...
	function assignDefaultCrate($property_uid = 0, $crate_id = 0)
		{
	...
	function getCurrentCrateInvoiceIdForManagerId($manager_id = 0)
		{
		if ( (int)$manager_id == 0 )
			return false;
	...
	function createNewCrateInvoiceForManagerId($manager_id = 0, $currencycode = 'GBP' )
		{
		if ( (int)$manager_id == 0 )
			{
