...
class JomresPage
	...
	function set_scroll_page( $scroll_num = 0 )
		{
		if ( $scroll_num != 0 ) $this->scroll_page = $scroll_num;
		else
	...
	function set_records_per_page( $records_per_page )
		{
		if ( $records_per_page <= 0 ) $records_per_page = $this->total_records;
	...
	function set_page_data( $page_name, $total_records, $records_per_page = 1, $scroll_num = 0, $show_prev_next = true, $show_scroll_prev_next = false, $show_first_last = false, $limit )
		...
		if ( isset( $_REQUEST[ 'ptype' ] ) ) $this->url_ptype_string = "&ptype=" . (int) $_REQUEST[ 'ptype' ];
	...
	function get_first_page_nav( $user_link = "", $link_para = "" )
		...
		if ( trim( $user_link ) == "" ) $user_link = "[" . _PN_START . "]&nbsp;";
		if ( !$this->is_first_page() && $this->show_first_last ) $txt .= ' <a href="' . jomresURL( $this->page_name . '&plistpage=0' . $this->qry_str ) . $this->url_ptype_string . '">' . $user_link . '</a> ';
		elseif ( $this->show_first_last && $this->show_disabled_links ) $txt .= $user_link;
	...
	function get_last_page_nav( $user_link = "", $link_para = "" )
		...
		$txt = "";
		if ( $this->total_page <= 1 ) return;
		if ( trim( $user_link ) == "" ) $user_link = "[" . _PN_END . "]";
		if ( !$this->is_last_page() && $this->show_first_last ) $txt .= ' <a href="' . jomresURL( $this->page_name . '&plistpage=' . ( $this->total_page - 1 ) . $this->qry_str ) . $this->url_ptype_string . '">' . $user_link . '</a> ';
		elseif ( $this->show_first_last && $this->show_disabled_links ) $txt .= '&nbsp;' . $user_link;
	...
	function get_number_page_nav( $link_para = "" )
		...
		if ( $this->page > ( $scroll_page / 2 ) ) $j = $this->page - intval( $scroll_page / 2 );
		if ( $j + $scroll_page > $this->total_page ) $j = $this->total_page - $scroll_page;
	...
	function calculate()
		...
		if ( $this->total_records % $this->records_per_page != 0 ) $this->total_page++;
		}
