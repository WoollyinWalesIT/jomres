...
function jomres_cmsspecific_getTextEditor($name, $content, $hiddenField, $width, $height, $col, $row)
	{
	return '<textarea class="inputbox" cols="40" rows="6" name="newtext">'.$content.'</textarea>';;
	/*
	$width="450";
	$height="250";
	$col="20";
	$row="10";
	$ret=editorAreaText( 'newtext', stripslashes($theText), 'newtext', $width, $height, $col, $row );
	return $ret;
	*/
	}
...
function jomres_cmsspecific_addheaddata($type,$path="",$filename="",$fullpathAndfilename="")
	...
	switch ($type) 
		{
		case "javascript":
			$tag = '<script type="text/javascript" src="'.$path.$filename.'"></script>';
			$mainframe->addCustomHeadTag($tag);
			//JHTML::script($filename, $path, false);
		break;
		case "css":
			$tag = '<link rel="stylesheet" href="'.$path.$filename.'" type="text/css" />';
			$mainframe->addCustomHeadTag($tag);
