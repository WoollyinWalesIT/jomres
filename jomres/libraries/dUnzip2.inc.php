...
class dUnzip2
		...
		if ( $this->debug )
			...
			foreach ( $this->compressedList as $fileName => $item )
				...
				foreach ( $item as $fieldName => $value )
					{
					if ( $fieldName == 'lastmod_datetime' ) echo "<td title='$fieldName' nowrap='nowrap'>" . date( "d/m/Y H:i:s", $value ) . "</td>";
					else
	...
	Function unzip( $compressedFileName, $targetFileName = false, $applyChmod = 0755 )
		{
		if ( !sizeof( $this->compressedList ) )
			{
		...
		if ( substr( $compressedFileName, -1 ) == JRDS )
			{
		...
		if ( $applyChmod && $targetFileName ) chmod( $targetFileName, 0755 );
	...
	Function unzipAll( $targetDir = false, $baseDir = "", $maintainStructure = true, $applyChmod = 0755 )
		{
		if ( $targetDir === false ) $targetDir = dirname( __FILE__ ) . JRDS;

		$lista = $this->getList();
		if ( sizeof( $lista ) ) foreach ( $lista as $fileName => $trash )
			{
			$dirname = dirname( $fileName );
			$outDN   = "$targetDir" . JRDS . "$dirname";
			if ( substr( $dirname, 0, strlen( $baseDir ) ) != $baseDir ) continue;
			if ( !is_dir( $outDN ) && $maintainStructure )
				{
				$str     = "";
				$folders = explode( JRDS, $dirname );
				foreach ( $folders as $folder )
					{
					$folder = str_replace( "/", JRDS, $folder );
					$str    = $str ? "$str" . JRDS . "$folder" : $folder;
					if ( !is_dir( $targetDir . JRDS . $str ) )
						{
						if ( $str != "." && $str != ".." )
							{
							$this->debugMsg( 1, "Creating folder: " . $targetDir . JRDS . $str );
							if ( !mkdir_recursive( $targetDir . JRDS . $str ) )
								{
								$this->debugMsg( 2, "Failed to create: " . $targetDir . JRDS . $str );

								return array ( "result" => false, "msg" => "Failed to create: " . $targetDir . JRDS . $str );
								}

							if ( $applyChmod ) chmod( $targetDir . JRDS . $str, $applyChmod );
							}
						}
					}
				}
			if ( substr( $fileName, -1, 1 ) == JRDS ) continue;

			$maintainStructure ? $this->unzip( $fileName, $targetDir . JRDS . $fileName, $applyChmod ) : $this->unzip( $fileName, $targetDir . JRDS . basename( $fileName ), $applyChmod );
			}

		return array ( "result" => true, "msg" => "Completed successfully " );
		}
	...
	Function debugMsg( $level, $string )
		{
		if ( $this->debug ) if ( $level == 1 ) echo "<b style='color: #777'>dUnzip2:</b> $string<br>";
		if ( $level == 2 ) echo "<b style='color: #F00'>dUnzip2:</b> $string<br>";
		}
	...
	Function _loadFileListByEOF( &$fh, $stopOnFile = false )
		...
		for ( $x = 0; $x < 1024; $x++ )
			...
			if ( $signature == $this->dirSignatureE )
				...
				$this->endOfCentral            = Array ( 'disk_number_this' => $eodir[ 'disk_number_this' ][ 1 ], 'disk_number' => $eodir[ 'disk_number' ][ 1 ], 'total_entries_this' => $eodir[ 'total_entries_this' ][ 1 ], 'total_entries' => $eodir[ 'total_entries' ][ 1 ], 'size_of_cd' => $eodir[ 'size_of_cd' ][ 1 ], 'offset_start_cd' => $eodir[ 'offset_start_cd' ][ 1 ], 'zipfile_comment' => $eodir[ 'zipfile_comment' ], );
	...
	Function _getFileHeaderInformation( &$fh, $startOffset = false )
		{
		if ( $startOffset !== false ) fseek( $fh, $startOffset );
