...
class dUnzip2{
	...
	Function unzipAll($targetDir=false, $baseDir="", $maintainStructure=true, $applyChmod=0755){
		if($targetDir === false)
			$targetDir = dirname(__FILE__).JRDS;
				...
				foreach($folders as $folder){
					...
					if(!is_dir($targetDir.JRDS.$str)){
						if ($str!="." && $str!=".." )
							...
							if($applyChmod)
								chmod($targetDir.JRDS.$str, $applyChmod);
							}
					}
				}
			}
			if(substr($fileName, -1, 1) == JRDS)
				continue;
			
			$maintainStructure?
				$this->unzip($fileName, $targetDir.JRDS.$fileName, $applyChmod):
				$this->unzip($fileName, $targetDir.JRDS.basename($fileName), $applyChmod);
		}
