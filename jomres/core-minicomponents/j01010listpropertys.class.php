		...
		if (count($propertyDeets) >0)
			...
			foreach ($propertyDeets as $property)
				...
				if ($cacheContent)
					...
					$property_deets['PROP_TOWN']='<a href="'.jomresURL(JOMRES_SITEPAGE_URL.'&send=Search&calledByModule=mod_jomsearch_m0&town='.htmlspecialchars(stripslashes(urlencode($propertyContactArray[2]))).'">'.stripslashes($propertyContactArray[2])).'</a>';
					$property_deets['PROP_POSTCODE']=stripslashes($propertyContactArray[3]);
					$property_deets['PROP_REGION']='<a href="'.jomresURL(JOMRES_SITEPAGE_URL.'&send=Search&calledByModule=mod_jomsearch_m0&region='.htmlspecialchars(stripslashes(urlencode($propertyContactArray[4])))).'">'.stripslashes($propertyContactArray[4]).'</a>';
					$property_deets['PROP_COUNTRY']='<a href="'.jomresURL(JOMRES_SITEPAGE_URL.'&send=Search&calledByModule=mod_jomsearch_m0&country='.htmlspecialchars(stripslashes(urlencode($propertyContactArray[5])))).'">'.stripslashes(stripslashes(getSimpleCountry($propertyContactArray[5]))).'</a>';
