...
class j16000viewproperty
    ...
    function j16000viewproperty( $componentArgs )
        ...
        if ( is_null( $id ) ) $id = jomresGetParam( $_REQUEST, 'id', 0 );

        if ( $id == 0 ) trigger_error( "Front end stats ID empty", E_USER_ERROR );
        ...
        for ( $y = $curYear; $y < $curYear + $numberOfYearsToShow; $y++ )
            ...
            for ( $m = 1; $m <= 12; $m++ )
                ...
                if ( !is_null( $bookings ) ) $bookingsArray[ $y ][ $m ][ ] = $bookings;
                }
            }
