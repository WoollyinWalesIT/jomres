	...
	function __construct()
		...
		if ( $owner != "%" )
			...
			if ( $owner != "%" )
				...
				if ( $owner == jr_gettext( '_JOMRES_MR_AUDIT_UNKNOWNUSER', _JOMRES_MR_AUDIT_UNKNOWNUSER, false ) ) $userid = '0';
				else
					{
		...
		foreach ( $auditList as $activity )
			...
			if ( count( $userDetails ) == 1 ) $rw[ 'USER' ] = $userDetails[ $id ][ 'username' ];
			else
