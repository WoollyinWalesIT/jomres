...
class j00030search
	...
	function __construct( $componentArgs )
		...
		if ( isset( $componentArgs[ 'doSearch' ] ) ) $doSearch = $componentArgs[ 'doSearch' ];
		if ( isset( $componentArgs[ 'includedInModule' ] ) ) $includedInModule = $componentArgs[ 'includedInModule' ];
		if ( isset( $componentArgs[ 'calledByModule' ] ) ) $calledByModule = $componentArgs[ 'calledByModule' ];
		...
		if ( in_array( "description", $searchOptions ) && $showSearchOptions )
			{
			if ( empty( $sch->filter[ 'description' ] ) ) $selectOption = $output[ 'JOMRES_SEARCH_DESCRIPTION_LABEL' ];
			else
				...
				if ( $searchOutput[ 'feature_uids' ] == "dropdown" )
					...
					foreach ( $sch->prep[ 'features' ] as $feature )
						...
						if ( is_array( $sch->filter[ 'feature_uids' ] ) )
							{
							if ( in_array( $pid, $sch->filter[ 'feature_uids' ] ) ) $ischecked = "checked";
							}
						$image = '/' . $feature[ 'image' ];
		...
		if ( $doSearch )
			...
			if ( $numberOfPropertiesInSystem > 1 && !$includedInModule && !isset( $_REQUEST[ 'calledByModule' ] ) && !isset( $_REQUEST[ 'next' ] ) && get_showtime( 'task' ) == "" )
				...
				if ( !empty( $sch->filter[ 'region' ] ) ) $sch->jomSearch_region();
				if ( !empty( $sch->filter[ 'town' ] ) ) $sch->jomSearch_town();
				if ( !empty( $sch->filter[ 'ptype' ] ) ) $sch->jomSearch_ptypes();
				if ( !empty( $sch->filter[ 'guestnumber' ] ) ) $sch->jomSearch_guestnumber();
				if ( !empty( $sch->filter[ 'stars' ] ) ) $sch->jomSearch_stars();


				if ( !empty( $sch->filter[ 'priceranges' ] ) ) $sch->jomSearch_priceranges();
				if ( !empty( $sch->filter[ 'feature_uids' ] ) ) $sch->jomSearch_features();
				if ( !empty( $sch->filter[ 'room_type' ] ) ) $sch->jomSearch_roomtypes();
				if ( !empty( $sch->filter[ 'description' ] ) ) $sch->jomSearch_description();
				if ( !empty( $sch->filter[ 'arrival' ] ) ) $sch->jomSearch_availability();
				}
			}
		if ( $showButton == true )
			{
