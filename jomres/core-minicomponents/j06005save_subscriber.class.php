...
class j06005save_subscriber
	...
	function j06005save_subscriber()
		...
		if ( !$thisJRUser->userIsRegistered )
			...
			if ( $trial_package_exists && $package_id == $trial_package_exists ) $subscribing_to_freebie = true;
			...
			if ( $subscriber->id > 0 ) $subscriber->commitUpdateSubscriber();
			else
			...
			if ( $package->getSubscriptionPackage() )
				...
				if ( !$subscribing_to_freebie ) $this->sendNewSubscription( $subscription, $subscriber, $invoice_handler->id, $invoice_handler->init_total, $invoice_handler->recur_total, $package->tax_code_id );
					...
					while ( $keeplooking ):
						...
						if ( count( $transaction ) == 0 ) $keeplooking = false;
						$dummy_txn_id = "XXXXXXXX DUMMY " . mt_rand( 10000000, 99999999 );
					endwhile;
	...
	function sendNewSubscription( $subscription, $subscriber, $invoice_id, $init_total, $recur_total, $tax_code_id )
		...
		{
			case 'M':
				if ( $subscription->trial_period > 0 ) $p1 = $subscription->trial_period * 1;
				$p3 = 1;
				break;
			case 'Q':
				if ( $subscription->trial_period > 0 ) $p1 = $subscription->trial_period;
				$p3 = 3;
				break;
			case 'B':
				if ( $subscription->trial_period > 0 ) $p1 = $subscription->trial_period;
