...
class j16000listpropertys
	...
	function j16000listpropertys()
		...
		foreach ( $jomresPropertyList as $k => $p )
			...
			$r = array ();
			$counter++;
			if ( $counter % 2 ) $r[ 'STYLE' ] = "row0";
			else
			$r[ 'STYLE' ] = "row1";

			if ( !in_array( $p[ 'id' ], $portalPropertyIds ) ) $r[ 'STYLE' ] = "row2";
			else
			...
			if ( !in_array( $p[ 'id' ], $portalPropertyIds ) ) $crid = $jrConfig[ 'defaultCrate' ];
			else
				{
				$id   = $k;
				$crid = $portalPropertyList[ $k ][ 'crate_id' ];
				}
			if ( is_null( $crid ) || $crid == 0 ) $r[ 'STYLE' ] = "row2";
			else
			$r[ 'STYLE' ] = "jradmin_field_la";
