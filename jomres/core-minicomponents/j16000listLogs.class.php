...
class j16000listLogs
	...
	function j16000listLogs()
		...
		$output['PAGETITLE']=jr_gettext("JOMRES_COM_A_AVAILABLELOGS",JOMRES_COM_A_AVAILABLELOGS);
		$counter=0;
		if (count($logFiles) > 0)
			{
		...
		if ($counter==0)
			echo jr_gettext("JOMRES_COM_A_LOGS_NOENTRIES",JOMRES_COM_A_LOGS_NOENTRIES);
		else
			{
			$pageoutput[]=$output;
