...
class j16000save_invoice
	...
	function j16000save_invoice()
		...
		foreach ( $p_line_item as $li_id )
			{
			$line_items[ ] = array ( 'id' => $li_id, 'init_qty' => (int) $p_init_qty[ $li_id ], 'init_discount' => (float) $p_init_discount[ $li_id ], 'recur_qty' => (int) $p_recur_qty[ $li_id ], 'recur_discount' => (float) $p_recur_discount[ $li_id ] );
			}
		...
		$task = "list_invoices";
		if ( $invoice_handler->is_commission == "1" ) $task = "list_commission_invoices";
