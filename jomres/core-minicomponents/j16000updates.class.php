...
class j16000updates
	...
	function is_removeable( $dir )
		{
		$folder = opendir( $dir );
		while ( $file = readdir( $folder ) ) if ( $file != '.' && $file != '..' && ( !is_writable( $dir . JRDS . $file ) || ( is_dir( $dir . JRDS . $file ) && !$this->is_removeable( $dir . JRDS . $file ) ) ) )
			{
			closedir( $dir );
		...
		while ( $file = readdir( $dirlist ) )
			...
			if ( $file != '.' && $file != '..' )
				...
				if ( $getWritablesFiles )
					{
					if ( is_file( $newpath ) && is_writable( $newpath ) ) $this->directoryScanResults[ ] = $dir . JRDS . $file;
					}
				else
					{
					if ( is_file( $newpath ) && !is_writable( $newpath ) ) $this->directoryScanResults[ ] = $dir . JRDS . $file;
	...
	function checkUpdateDirectory()
		...
		if ( !is_dir( $this->updateFolder ) )
			...
			{
			if ( !is_writable( $this->updateFolder ) ) return false;
			}
	...
	function emptyDir( $dir )
		...
		closedir( $dh );
		if ( $dir != $this->updateFolder ) @rmdir( $dir );
		}
