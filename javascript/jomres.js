...
(function(jomresJquery) {
    jomresJquery.fn.shorten = function(settings) {
        "use strict";
        ...
        var config = {
            ...
            minHideChars: 10,
            ellipsesText: "...",
            moreText: jomres_javascript_readmore,
            lessText: jomres_javascript_readless,
            onLess: function() {},
            onMore: function() {},
            errMsg: null,
            force: false
        ...
        if (settings) {
            jomresJquery.extend(config, settings);
        }

        if (jomresJquery(this).data('jquery.shorten') && !config.force) {
            return false;
        }
        jomresJquery(this).data('jquery.shorten', true);

        jomresJquery(document).off("click", '.morelink');

        jomresJquery(document).on({
            click: function() {
        ...
        return this.each(function() {
            ...
            if (contentlen > config.showChars + config.minHideChars) {
                ...
                if (c.indexOf('<') >= 0) // If there's HTML don't want to cut it
                {
                    ...
                    c = jomresJquery('<div/>').html(bag + '<span class="ellip">' + config.ellipsesText + '</span>').html();
                }else{
                ...
                jomresJquery('.shortcontent p:last', $this).css('margin-bottom', 0); //Remove bottom margin on last paragraph as it's likely shortened
            }
        });
