...
function loader_already_installed($rtl = null)
{
	...
	if ($newer_version) {
		echo '<pclass="alert alert-danger">The ionCube Loader version ' . $lv . $php_str . ' is <strong>already installed</strong> but it is an old version.';
		...
		echo '<p class="alert alert-success">The ionCube Loader version ' . $lv . $php_str . ' is already installed and encoded files should run without problems.</p>'; 
		echo '</div>';
...
{
	$leg_warn = '<p class="alert alert-danger"><strong>You are on a platform on which ionCube Loaders are no longer being developed. ';
...
function loader_not_installed()
{
	...
	if (!isset($stype)) {
		echo '<p class="alert alert-info">To use files that have been protected by the <a href="' . ENCODER_URL . '" target=encoder>ionCube PHP Encoder</a>, a component called the ionCube Loader must be installed.</p>';
	}

	if (!is_supported_php_version()) {
		echo '<p class="alert alert-danger">Your server is running PHP version ' . PHP_VERSION . ' and is
		...
		echo '<strong>Your server is running PHP version ' . PHP_VERSION . ' and is
		 ...
		 echo '<p class="alert alert-danger">Your server is currently running a debug build of PHP. The Loader cannot be installed with a debug build of PHP. Please ensure that PHP is reconfigured with debug disabled. Note that debug builds of PHP cannot help in debugging PHP scripts.</p>'; 
	} elseif (!is_array($loader)) {
		if ($loader == ERROR_WINDOWS_64_BIT) {
			echo '<p class="alert alert-danger">Loaders for 64-bit PHP on Windows are not currently available. However, if you <b>install and run 32-bit PHP</b> the corresponding 32-bit loader for Windows should work.</p>';
			if ($sysinfo['THREAD_SAFE']) {
...
/* 	$page = '';
	...
	return JOMRES_SITEPAGE_URL_ADMIN.'&task=loader_wizard';
}
