...
class j01009a_filterproperties
	...
	function __construct( $componentArgs )
		...
		if ( !isset( $_REQUEST[ 'jomsearch_sortby' ] ) && isset( $tmpBookingHandler->user_settings[ 'jomsearch_sortby' ] ) ) $sortid = $tmpBookingHandler->user_settings[ 'jomsearch_sortby' ];
		else
			{
			if ( isset( $_REQUEST[ 'jomsearch_sortby' ] ) ) $sortid = intval( jomresGetParam( $_REQUEST, 'jomsearch_sortby', 1 ) );
			else
		...
		switch ( $sortid )
		{
			...
			case '3':
				...
				foreach ( $regions as $u ) $this->propertys_uids[ ] = $u->propertys_uid;
				break;
			#########################################################################################
			case '4':
