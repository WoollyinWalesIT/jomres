...
class j06000cron_invoice
		...
		if ( count( $items ) > 0 )
			...
			foreach ( $items as $key => $data )
				...
				foreach ( $data as $d )
					...
					$line_item_data     = array ( 'tax_code_id' => $d[ 'tax_code_id' ], 'name' => $d[ 'name' ], 'description' => $d[ 'description' ], 'init_price' => $d[ 'init_price' ], 'init_qty' => $d[ 'init_qty' ], 'init_discount' => $d[ 'init_discount' ], 'recur_price' => $d[ 'recur_price' ], 'recur_qty' => $d[ 'recur_qty' ], 'recur_discount' => $d[ 'recur_discount' ] );
	...
	function suspend( $managers_id )
		{
		if ( (int) $managers_id == 0 ) return false;
