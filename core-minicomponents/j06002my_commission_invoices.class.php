		...
		if ($thisJRUser->userIsManager)
			...
			if (count($invoices)>0)
				...
				if (!using_bootstrap())
					...
					//if ($thisJRUser->userIsManager)
						//{
						$output['TASK_FILTER_ANY']='<a href="'.JOMRES_SITEPAGE_URL_ADMIN.'&task=my_commission_invoices">'._JOMRES_FRONT_ROOMSMOKING_EITHER.'</a>';
						$output['TASK_FILTER_UNPAID']='<a href="'.JOMRES_SITEPAGE_URL_ADMIN.'&task=my_commission_invoices&status=unpaid">'._JRPORTAL_INVOICES_STATUS_UNPAID.'</a>';
						$output['TASK_FILTER_PAID']='<a href="'.JOMRES_SITEPAGE_URL_ADMIN.'&task=my_commission_invoices&status=paid">'._JRPORTAL_INVOICES_STATUS_PAID.'</a>';
						$output['TASK_FILTER_CANCELLED']='<a href="'.JOMRES_SITEPAGE_URL_ADMIN.'&task=my_commission_invoices&status=cancelled">'._JRPORTAL_INVOICES_STATUS_CANCELLED.'</a>';
						$output['TASK_FILTER_PENDING']='<a href="'.JOMRES_SITEPAGE_URL_ADMIN.'&task=my_commission_invoices&status=pending">'._JRPORTAL_INVOICES_STATUS_PENDING.'</a>';
						//}
					//else
//						{
