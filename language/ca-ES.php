...
jr_define ('_JOMRES_DEFAULT_SHORTCODE_ALERT', 'Assegureu-vos que heu creat una pàgina de WordPress que contingui el codi curt [jomres: xx-XX] (on xx-XX és el codi d\'idioma del lloc, per exemple [jomres: en-GB] o [ jomres: ca-US]) en cas contrari, no podreu accedir a Jomres des del frontend per gestionar les vostres propietats i reserves. <br> Jomres està basat en el marc Bootstrap, de manera que l’heu d’utilitzar en un tema basat en Bootstrap. recomanem un basat en Bootstrap 3. Si no teniu accés a un tema, us recomanem que utilitzeu el tema Jomres Leohtian per a Wordpress, que podeu <a href = "https://www.jomres.net/download/free-downloads/download/5-miscelleneous-downloads/11-leohtian-for-wordpress "target =" _blank "> download from here. </a> ');
...
jr_define ('_JOMRES_FAQ_ADMIN_CATEGORY_INTRODUCTION', 'Introducció');
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_INTRODUCTION_WHATISJOMRES', 'Què és Jomres?');
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_INTRODUCTION_WHATISJOMRES", "Jomres és un sistema de reserva d'hotels i xalets que funciona amb els sistemes de gestió de continguts de Joomla i Wordpress. És un sistema de reserves complet, penseu\" booking.com \"o\" airbnb \"sense despeses. És un conjunt d'eines complet. que us permetrà crear un servei de reserva en línia únic per a tantes propietats com vulgueu. ");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_INTRODUCTION_USERSADD', 'Els usuaris poden registrar-se al meu lloc de Jomres i afegir les seves pròpies propietats?');
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_INTRODUCTION_USERSADD", "Sí. Per defecte, Jomres permet als usuaris registrats afegir les seves pròpies propietats al vostre lloc. Podeu desactivar-ho a la pestanya Configuració del lloc> Diversos si cal.");
jr_define ('_JOMRES_FAQ_ADMIN_CATEGORY_PROPERTIES', 'Propietats');
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_ADDPROPERTIES', 'Com puc afegir una propietat nova?');
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_ADDPROPERTIES", "Quan s'instal·la Jomres, l'usuari administrador es crea com a Super Property Manager. Inicieu sessió a la portada del vostre CMS (les pàgines públiques) com a usuari administrador i des d'allà podeu afegir noves propietats. <br / > Podeu crear administradors nous a la pàgina Administradors de propietats si voleu afegir-ne un de nou, però, de manera predeterminada (podeu desactivar-ho), qualsevol usuari registrat que creï una propietat es crea automàticament com a administrador de propietats per a les propietats que creen. per raons de seguretat, no poden administrar les propietats d'altres administradors. Això es pot canviar elevant aquest usuari a Super Administrador de propietats a la pàgina Administradors de propietats. ");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_IMPORTPROPERTIES', 'Puc importar propietats d\'un altre sistema?');
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_IMPORTPROPERTIES", "Hi ha diverses maneres d\'importar propietats. El connector Importació de propietats us permet importar propietats mitjançant CSV. Això us permet importar diverses propietats alhora, però només és una característica d'administrador. Si la propietat gestor té un compte de Beds24 i després pot importar qualsevol propietat existent del seu compte un cop hagi enllaçat les claus de l'API. Una altra manera és utilitzar l'API, hi ha una característica per afegir i eliminar propietats, però això requeriria que sigueu desenvolupador. està familiaritzat amb l'ús d'API REST per utilitzar aquesta funcionalitat. ");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_IMPORTBOOKINGS', 'Puc importar reserves d\'un altre sistema?');
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_IMPORTBOOKINGS", "Les administracions poden importar les reserves a una propietat mitjançant el connector iCal, que ofereix una funció per importar les reserves al menú del gestor de propietats. A més, si un administrador de propietats té un compte Beds24 i la seva propietat té s’han enllaçat amb Beds24 i després poden fer una acció de reenviament de notificació a Beds24. ");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_EMAILTEMPLATES', 'Puc editar plantilles de correu electrònic?');
jr_define ('_JOMRES_FAQ_ADMIN_ANSWER_EMAILTEMPLATES', 'Els administradors de propietats poden editar plantilles de correu electrònic a través de la seva barra d\'eines a Configuració> Plantilles de correu electrònic');
jr_define ('_JOMRES_FAQ_ADMIN_CATEGORY_PAYMENTS', 'Pagaments');
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_TAKEPAYMENTS', 'Com puc permetre que les propietats facin pagaments?');
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_TAKEPAYMENTS", "Haureu d'instal·lar una passarel·la de pagament. Jomres té dues passarel·les de pagament que es connecten a Paypal o Stripe. També ofereix un parell de passarel·les fora de línia i podeu comprar altres passarel·les de pagament als nostres socis." );
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_WHICHGATEWAY', 'Quina passarel·la he d\'utilitzar?');
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_WHICHGATEWAY", "Si voleu obtenir un percentatge del dipòsit d'un gestor per als vostres propis costos, utilitzeu la passarel·la Stripe. Si no ho feu i, en canvi, voleu utilitzar la funcionalitat de subscripció i / o comissió, Hauré d'utilitzar la passarel·la de Paypal. ");
jr_define ("_JOMRES_FAQ_ADMIN_QUESTION_PAYMENTACCOUNTS", "Quan un hoste fa un pagament per una reserva, qui obté els diners?");
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_PAYMENTACCOUNTS", "Això depèn de com hàgiu configurat les passarel·les de pagament al vostre lloc. Si utilitzeu Paypal, podeu configurar-lo per substituir tots els paràmetres de frontend, essencialment convertint el vostre compte en l'únic compte que rep pagaments. Si si no escolliu fer-ho, cada propietat haurà de configurar el seu compte de passarel·la de pagament a Configuració> pestanya Passarel·les. Si utilitzeu Stripe, un cop el gestor de propietats hagi connectat el seu compte al vostre (vegeu la documentació de Stripe a el manual per obtenir més informació) rebran tots els fons, menys una comissió que podeu configurar si decidiu fer-ho. ");
jr_define ('_JOMRES_FAQ_ADMIN_CATEGORY_TROUBLESHOOTING', 'Solució de problemes');
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_TROUBLESHOOTING_EMAIL', "No s'envien correus electrònics de reserva.");
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_TROUBLESHOOTING_EMAIL", "Si teniu problemes per enviar correus electrònics des del sistema, consulteu la configuració de correu electrònic del vostre CMS amfitrió. Jomres utilitza la configuració de Wordpress o Joomla per enviar correus electrònics. Si estan equivocats, Jomres no podrà envieu correus electrònics. ");
...
jr_define ('_JOMRES_FAQ_ADMIN_ANSWER_TROUBLESHOOTING_NOGATEWAY', "Si heu iniciat la sessió com a administrador de propietats, no veureu la passarel·la de pagaments, ja que no pagueu a vosaltres mateixos, ximple. Només els usuaris que no siguin gestors veuran la passarel·la en fer les reserves.") ;
jr_define ('_JOMRES_FAQ_ADMIN_CATEGORY_PURCHASINGJOMRES', 'Llicència Jomres');
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_PURCHASINGJOMRES_FORCEDTOSUBSCRIBE', 'Després de comprar la llicència X, estic obligat a comprar renovacions un cop hagi caducat la llicència?');
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_PURCHASINGJOMRES_FORCEDTOSUBSCRIBE", "No, si heu comprat una llicència Starter, Business o Enterprise, podeu continuar utilitzant el programari indefinidament, no es restringirà de manera màgica. L'excepció a aquesta regla és si adquireu una subscripció. Un cop cancel·lada la subscripció, ja no podreu utilitzar els connectors i els haureu de desinstal·lar. ");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_PURCHASINGJOMRES_EXPIRED', 'La meva llicència ha caducat, puc actualitzar Jomres de totes maneres?');
jr_define ('_JOMRES_FAQ_ADMIN_ANSWER_PURCHASINGJOMRES_EXPIRED', "Tot i que pugueu físicament, no és recomanable que ho feu. Un cop hagi caducat la llicència, ja no podreu beneficiar-vos del suport ni descarregar actualitzacions dels connectors per a la vostra instal·lació de Jomres. Això vol dir que hauríeu de < i> not </i> actualitzeu la vostra instal·lació de Jomres després que la llicència hagi caducat. Això és degut a que els connectors s'han d'actualitzar regularment per funcionar amb les versions actuals del sistema, de manera que sovint els connectors antics no funcionaran en una versió més nova del sistema . Si encara voleu actualitzar per aprofitar les noves funcionalitats, hauríeu de comprar una llicència de renovació, que és efectivament una llicència normal amb un descompte del 50%. ");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_PURCHASINGJOMRES_SOFTWARELICENSE', "Amb quina llicència de programari es publica Jomres?");
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_PURCHASINGJOMRES_SOFTWARELICENSE", "Jomres té llicència tant sota les llicències MIT com GPL. Els connectors descarregats per les llicències Starter, Business i Enterprise es publiquen sota la mateixa llicència i són completament gratuïts i de codi obert. Els connectors descarregats per a proves i llicències de subscripció són. no llançats sota les llicències GPL o MIT i estan codificats Ioncube. Aquest enfocament ens permet oferir-vos una varietat de llicències diferents per adaptar-se a les butxaques de tothom. ");
...
jr_define ("_JOMRES_FAQ_MANAGER_ANSWER_DISCOUNTS", "Es poden fer descomptes, hi ha diverses maneres de fer-ho. Si feu una reserva en nom d'un client, podeu establir els vostres propis dipòsits i reserves al formulari de reserva , mitjançant els camps \"Substitueix l'allotjament total \" i \"Substitueix el dipòsit \" (els convidats no poden utilitzar aquesta funció). Una altra manera de donar un descompte a un hoste és crear cupons de descompte, que es poden configurar perquè puguin només es pot utilitzar entre determinades dates (vàlid de / a) o s'aplica només quan la reserva es troba entre determinades dates (reserva vàlida de / a). Aquest cupó de descompte es pot assignar a un sol convidat o, si voleu, podeu imprimir el cupons. La impressió inclou un codi QR que els clients poden escanejar als seus telèfons que els portarà al formulari de reserva amb el codi de descompte que ja s'aplica. ");
...
jr_define ('_JOMRES_FAQ_MANAGER_ANSWER_BOOKINGS_BLACK', "Les reserves en negre són reserves que s'han creat per deixar una o més habitacions fora de servei. No s'associen a cap convidat i són útils, per exemple, si cal reformar una habitació.") ;
...
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_CURRENCIES_SETUP', "Com puc configurar els símbols de moneda?");
jr_define ('_JOMRES_FAQ_ADMIN_ANSWER_CURRENCIES_SETUP', "No ho feu. Tots els símbols de moneda ja estan emmagatzemats al sistema i correspon als administradors de propietats seleccionar la moneda adequada per a ells a la pàgina Configuració de propietats.");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_CURRENCIES', 'Com puc habilitar diverses monedes?');
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_CURRENCIES", "Jomres té la funcionalitat de convertir els preus d'una moneda a una altra automàticament. Primer heu de visitar la pestanya Configuració del lloc> Conversió de divises. Allà hi trobareu enllaços a dos serveis en línia. El primer servei, un cop el L'API s'ha desat, permet a Jomres baixar dades de conversió de divises un cop al dia. El segon permet la geolocalització basada en IP per als usuaris. Demaneu una clau API gratuïta a cada servei. Amb aquests dos serveis combinats, Jomres pot detectar la moneda local de l'usuari i mostrar-la. preus de les propietats tant en la moneda local del client, com en la moneda de l'hotel. Si no configureu aquestes claus API, Jomres passarà al símbol de moneda euro per defecte. Si voleu, podeu establir un codi de moneda global. Això significa que la propietat els gestors no podran seleccionar cap altra moneda, però els clients potencials encara veuran els preus en la seva moneda local, tret que hagin utilitzat la funció de canvi de moneda per canviar la moneda en què es mostren els preus. ");
jr_define ('_JOMRES_FAQ_MANAGER_QUESTION_LANGUAGES', 'Com puc desar descripcions en diversos idiomes?');
jr_define ("_JOMRES_FAQ_MANAGER_ANSWER_LANGUAGES", "Per desar descripcions en diversos idiomes, primer visiteu la pàgina Configuració> Detalls de la propietat. Deseu allà la descripció de la vostra propietat. A continuació, canvieu l'idioma en què esteu veient el lloc. Ara aneu a Configuració Torneu a la pàgina Detalls de la propietat i torneu a desar-los. Per tant, si el lloc és capaç de mostrar tant els idiomes anglès com espanyol (o qualsevol altre), seleccioneu anglès, introduïu la descripció en anglès i feu clic a Desa. A continuació, canvieu la vostra pàgina actual idioma a l'espanyol i, a continuació, deseu la nova descripció espanyola. Funciona per a totes les entrades d'aquesta pàgina. ");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_PROPERTIES_NUMBER', 'Quantes propietats puc llistar?');
jr_define ('_JOMRES_FAQ_ADMIN_ANSWER_PROPERTIES_NUMBER', "No hi ha cap límit codificat per al nombre de propietats que pugueu enumerar, l'única limitació es troba en la limitació física del propi servidor. Si es tracta d'un servidor amb molts altres comptes, els recursos seran limitats, si , d'altra banda és un servidor dedicat o virtual dedicat, llavors podreu enumerar moltes més propietats. ");
...
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_INTRODUCTION_PROFILES', 'Puc convertir un usuari en administrador de propietats?');
jr_define ('_JOMRES_FAQ_ADMIN_ANSWER_INTRODUCTION_PROFILES', "apareix a sota. Feu clic al seu nom d'usuari i se us dirigirà a una pàgina on podreu configurar a quines propietats tenen accés. Si encara no estan registrats, primer haureu d'utilitzar les pàgines de gestió d'usuaris del vostre CMS amfitrió per afegir-les com a usuari del CMS. <br/> <br/> Un consell: si teniu intenció de cobrar als administradors de propietats per llistar les seves propietats al vostre lloc, ja sigui mitjançant la comissió a través de la passarel·la Stripe o la funcionalitat de subscripcions / comissions, no us recomanem que afegiu usuaris d'aquesta manera. En aquest cas, només s'haurien d'afegir superadministradors de propietats mitjançant aquest procés i, quan s'afegeixin, no heu de seleccionar cap propietat a la llista de sota. Cal animar els usuaris a crear propietats ells mateixos al frontend. Això els afegirà automàticament com a administradors de propietats i s'assegurarà que es facturi a l'usuari correcte. ");
jr_define ('_JOMRES_FAQ_ADMIN_CATEGORY_PLUGINS', 'Connectors');
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_GENERAL_PLUGINS', 'Per a què serveix el gestor de connectors?');
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_GENERAL_PLUGINS", "El gestor de connectors és una eina clau de Jomres. Ens permet distribuir-vos la versió més actualitzada dels connectors Jomres mitjançant una funció d'instal·lació i descàrrega automàtica");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_GENERAL_PLUGINS_IONCUBE', 'Per què veig un advertiment sobre els carregadors Ioncube quan faig clic a la pàgina Administrador de connectors?');
jr_define ('_JOMRES_FAQ_ADMIN_ANSWER_GENERAL_PLUGINS_IONCUBE', "Heu desat una clau de llicència de prova a la configuració del lloc, però el vostre servidor web no ofereix els darrers carregadors Ioncube. Demaneu als vostres amfitrions que instal·lin els carregadors, que es poden trobar <a href = 'http : //www.ioncube.com/loaders.php 'target =' _blank '> en aquesta pàgina. </a> Si heu comprat una llicència completa sense subscripció i creieu que ho veieu per error, pot haver oblidat introduir la vostra clau de llicència completa a la pàgina de configuració del lloc. ");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_GENERAL_PLUGINS_INSTALLATION', 'Com puc instal·lar un connector?');
jr_define ('_JOMRES_FAQ_ADMIN_ANSWER_GENERAL_PLUGINS_INSTALLATION', "Suposant que teniu un número de clau de llicència vàlid desat a la configuració del lloc, tot el que heu de fer és fer clic al botó Instal·la al costat del nom del connector al gestor de connectors. Jomres us instal·larà el connector i el portarà torneu a la pàgina de connectors després. De tant en tant, és possible que us dirigiu a una pàgina de descobriment si executeu Jomres a Joomla. Feu clic al botó Descobriu per instal·lar el connector. <br/> <br/> NOTA No ho feu No instal·leu tots els connectors, no funcionen tots junts (segons el disseny). Podeu acabar amb pàgines en blanc al formulari de reserva. En lloc d'això, identifiqueu els connectors que necessiteu i experimenteu-los. ");
jr_define ('_JOMRES_FAQ_ADMIN_CATEGORY_SITESTRUCTURE', 'Estructura del lloc');
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_SITESTRUCTURE_INTRO', 'Què significa Estructura del lloc?');
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_SITESTRUCTURE_INTRO", "L'estructura del lloc a Jomres fa referència als diversos elements del vostre lloc que el fan únic. Defineix els tipus de propietats que enumereu (per exemple, xalets o hotels) i els tipus / habitacions d'aquests tipus de propietats. Per tant, , si veneu vacances amb cuina, vendríeu lloguers de xalet o xalet. Xalets diferents poden tenir un nombre diferent d'habitacions, balcons, etc. En el cas dels hotels o bed and breakfasts, veneu habitacions <em> a </em> la propietat, i serien habitacions dobles, individuals, etc. Per ajudar-vos a aclarir-ho, una instal·lació predeterminada ja té diverses habitacions i tipus de propietats creades per ajudar-vos a entendre-les. Si voleu vendre Tours, no intenteu crear un tipus de propietat nou sense llegir primer la secció sobre Tours. ");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_SITESTRUCTURE_PROPERTYTYPES', "Què són els tipus de propietat?");
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_SITESTRUCTURE_PROPERTYTYPES", "En aquest sistema, un tipus de propietat fa referència, literalment, al tipus de propietat. Pot ser un hotel, un xalet o un apartament. Els administradors de propietats no poden crear tipus de propietats perquè els tipus de propietats són un element que pot seran cercats pels convidats. Per assegurar-vos que la vostra funcionalitat de cerca sigui agradable, ordenada i ordenada, només els administradors del lloc poden crear tipus de propietats. ");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_SITESTRUCTURE_PROPERTYFEATURES', 'Què són les característiques de la propietat?');
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_SITESTRUCTURE_PROPERTYFEATURES", "Les característiques de la propietat són coses que fan que una propietat sigui única. Per exemple, una característica de la propietat pot ser que sigui\" A prop de la platja \", mentre que un altre hotel pugui tenir la característica que es troba dins d'un parc nacional, ofereix ponts, una cosa així. Igual que els tipus de propietats, els administradors no poden crear funcions noves, sinó que només poden seleccionar entre les que creeu. Les funcions de propietat poden tenir assignades imatges. Primer heu de penjar imatges de funcions a través de Site Strucutre> Media Center> Funció de propietat Menú desplegable Icones. Pengeu tantes icones de funcions com vulgueu, i després, quan creeu una Funció de propietat, podeu escollir la imatge. ");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_SITESTRUCTURE_PROPERTYFEATURECATEGORIES', "Què són les categories de característiques de propietat?");
jr_define ('_JOMRES_FAQ_ADMIN_ANSWER_SITESTRUCTURE_PROPERTYFEATURECATEGORIES', "Les funcions de la propietat es mostren a la pàgina Detalls de la propietat, entre d'altres. A la pàgina Detalls de la propietat, es poden mostrar les funcions agrupades per categories, per exemple\" Àrea d\'estar \",\" Accessibilitat \", etc.");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_SITESTRUCTURE_ROOMFEATURES', 'Què són les funcions de l\'habitació?');
jr_define ('_JOMRES_FAQ_ADMIN_ANSWER_SITESTRUCTURE_ROOMFEATURES', "Els administradors de propietats que utilitzen el mode d'edició tarifària Micromanage o Advanced poden editar les seves habitacions directament. També poden crear funcions d'habitacions que es mostren a la pàgina de disponibilitat d'habitacions per a cada propietat. Si utilitzeu la pantalla de la llista d'habitacions clàssiques al formulari de reserva i, a continuació, els clients poden utilitzar les funcions de l'habitació per filtrar les habitacions abans de seleccionar-ne una o més. A la pàgina Estructura del lloc> Funcions de l'habitació, els gestors del lloc també poden crear funcions de l'habitació i aquestes funcions. es pot veure a tots els formularis de reserva de propietats on la propietat utilitza la llista d'habitacions clàssiques. Bàsicament, es tracta d'una funció d'habitació global. Aquestes funcions de l'habitació es poden configurar per mostrar-les només a determinats tipus de propietats i les imatges d'aquestes funcions es poden penjar mitjançant la pàgina Estructura del lloc> Media Center. ");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_SITESTRUCTURE_COUNTRIES', 'Per què hi ha una pàgina per editar països / regions?');
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_SITESTRUCTURE_COUNTRIES", "Pensareu que una llista de països era bastant immutable, però us equivocaríeu. Al llarg dels anys, en mantenir una llista de països, hem vist molts canvis. Com a resultat, Vaig saber que era més fàcil permetre als gestors de llocs definir els països que els agradaria mostrar al lloc. Això permet al català resistent mostrar la seva província com un país únic, si aquesta és la declaració que voldrien fer. El mateix passa amb altres regions del món. ");
jr_define ('_JOMRES_FAQ_ADMIN_CATEGORY_LOCALISATION', 'Localització / Traducció');
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_LOCALISATION_INTRO', 'Com puc traduir les coses?');
jr_define ('_JOMRES_FAQ_ADMIN_ANSWER_LOCALISATION_INTRO', "La traducció d'etiquetes / cadenes es realitza a través de les opcions del menú Idiomes, principalment a les pàgines Tradueix el fitxer d'idioma / Tradueix les ubicacions locals. Com que els fitxers d'idioma són molt grans, quan feu clic en aquesta pàgina, poden trigar una estona a carrega. <br/> <br/> Vegem un exemple. Suposem que vols traduir Alemanya a Alemanya. Primer fes clic a Traducció de llengües locals. A continuació, comprova el menú desplegable d'idioma a la part superior dreta de l'anglès a l'alemany. A continuació, desplaça't cap avall fins arribar a Alemanya. Podeu veure que té un subratllat puntejat. Això vol dir que el podeu traduir. Feu clic a la paraula, veureu una finestra emergent que conté la paraula \"Alemanya\". Canvieu-la a \"Deutschland\" i feu clic a la marca verda. Si el vostre CMS amfitrió està configurat per oferir pàgines en alemany als visitants del lloc, la paraula \"Alemanya\" ara es mostrarà com a \"Deutschland\". Aquest concepte s'aplica a Jomres. <br/> <br/> A les pàgines del Gestor de propietats, també podrien ser els Administradors de propietats vegeu un menú desplegable del mode d'edició d'etiquetes a la part superior de les seves pàgines. Quan estigui activat, també podran fer canvis al frontend, però aquests canvis només afectaran la seva propietat activa actual, de manera que, per exemple, poden canviar fàcilment la paraula Tarifes a Preus per una sola propietat si això és el que és obligatori. ");
jr_define ('_JOMRES_FAQ_ADMIN_CATEGORY_TOURS', 'Tours');
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_TOURS_INTRO', 'Què són les excursions?');
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_TOURS_INTRO", "Pot semblar una pregunta ximple, però de fet no ho és. Al llarg dels anys hem après que les diferents cultures tenen percepcions diferents del que és una\" gira \". Com a resultat, les expectatives de vendre gires varien molt, de manera que abans de discutir sobre les rutes, primer definirem què pensem de les rutes. <br/> Un recorregut és un viatge senzill o una visita per veure una atracció o un monument. Tanmateix, una estada a un hotel no és un recorregut. es poden participar visites turístiques allotjades en un hotel. ");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_TOURS_HOWTOSELL', 'Com puc utilitzar Jomres per vendre excursions?');
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_TOURS_HOWTOSELL", "Per abordar la qüestió de la venda de tours a Jomres, primer haureu d'instal·lar el connector Jintour. Aquest connector us permet crear visites que es venen com a lloguer de vacances. També us permet crear propietats que s'utilitzen exclusivament per vendre tours. Un cop instal·lat aquest connector, els administradors de propietats podran crear Tours i propietats de Tour al frontend.");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_TOURS_WHATISJINTOUR', 'Què és el complement Jintour?');
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_TOURS_WHATISJINTOUR", "Jintour significa Jomres INtegrated TOURs. Us permet crear recursos limitats i senzills que es poden reservar, que no són habitacions d'una propietat. Tot i que el connector està dissenyat per permetre principalment les reserves de visites, es poden utilitzar per a moltes coses, no només per a excursions. Poden ser utilitzats, per exemple, per un allotjament d’esquí que vulgui llogar esquís i material. Com que aquest equipament tindrà una disponibilitat limitada, podeu utilitzar el connector Jintour per permetre als hostes reserveu i pagueu aquest material al moment de la reserva.");
jr_define ("_JOMRES_FAQ_ADMIN_QUESTION_TOURS_SAMEBOOKING", "Com puc vendre excursions i lloguers de vacances a la mateixa reserva?");
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_TOURS_SAMEBOOKING", "En aquest escenari, teniu un hotel i voleu vendre excursions a una atracció local. Inicieu sessió al frontend i aneu a l'hotel que heu creat. A continuació, feu clic a Configuració> Visita / Gestió de l'activitat. En aquesta pàgina, seguiu les instruccions que apareixen a la pantalla per crear primer un perfil de viatge i, a continuació, generar visites guiades. Un cop fet això, podreu vendre excursions i reserves d'habitacions i extres opcionals junts al mateix formulari de reserva. . ");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_TOURS_STANDALONETOURS', 'Què passa si només vull vendre tours?');
jr_define ("_JOMRES_FAQ_ADMIN_ANSWER_TOURS_STANDALONETOURS", "Primer haureu de crear una nova propietat al frontend. Quan ho feu, assegureu-vos que definiu el\" Tipus de propietat \"a Tour. Quan hàgiu creat aquesta nova propietat, podreu per tornar a crear visites a través de la pàgina Configuració> Gestió de visites / activitats. També podreu oferir extres opcionals com en les propietats normals, l'únic que no podreu fer és configurar les habitacions, les funcions de l'habitació o les tarifes tal com feu . no es venen habitacions en aquesta propietat. ");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_TOURS_GLOBALTOURS', 'Com puc crear un recurs Tourable / reservable que es pugui veure en tots els formularis de reserva de propietats?');
jr_define ('_JOMRES_FAQ_ADMIN_ANSWER_TOURS_GLOBALTOURS', "A l'àrea d'administrador del vostre lloc, visiteu la pàgina Portal> Functionality Tour / Activity Managenent page. El procés de generació de perfils / recorreguts és el mateix que s'utilitza al frontend, tot i que els recorreguts generats aquí es poden veure a la Un dels escenaris típics en què es pot fer servir és si l'administrador del lloc organitza esdeveniments a tot el país i vol vendre reserves d'hotels. Crearien els hotels participants al frontend amb normalitat, establint els preus de les habitacions, etc. les visites en aquest cas serien en realitat la quota d'assistència a l'esdeveniment en si mateix.");
jr_define ('_JOMRES_FAQ_ADMIN_QUESTION_INTRODUCTION_FIRSTTHINGSFIRST', "D'acord, ja he instal·lat Jomres, què després?");
jr_define ('_JOMRES_FAQ_ADMIN_ANSWER_INTRODUCTION_FIRSTTHINGSFIRST', "El primer que heu de fer, ara que heu instal·lat Jomres, és fer clic a Ajuda al menú de l'esquerra i, a continuació, fer clic a Introducció. De debò, aneu i feu-ho ara.") ;
