...
jr_define('JOMRES_COM_A_TARIFFMODE_DESC', "<b>Попередження: перемикання між різними режимами редагування тарифів може призвести до втрати даних. Про це див. примітку нижче.</b>.
<br/><br/>
...
jr_define('JOMRES_COM_A_MAPSKEY_DESC', 'Ви можете отримати ключ API Google Maps з <a href = "https://console.cloud.google.com/projectselector2/google/maps-apis/credentials" target="_blank"> Карти Google </a>. Після того, як ви введете тут ключ своєї карти, Джомрес покаже карту на вашій сторінці" Подробиці власності ". ');
...
jr_define('_JOMRES_COM_LANGUAGE_CONTEXT_DESC', "Використовуйте цей параметр, щоб змінити мовний контекст сайту. Ця функція дозволяє Jomres використовувати мітки, які відповідають тому, як ви використовуєте Jomres, тому, якщо ви зосереджені на брокерській діяльності на яхтах, зміна контексту дозволить Jomres надавати мітки, які відповідають яхтам. Наприклад, якщо ви встановите контекст як «Yacht Brokerage», тоді Jomres спочатку знайде поточну мову, а потім здійснить пошук рядків у користувацькій текстовій базі даних, збережених із мовним контекстом «yachtbrokerage». Якщо спеціальні рядки для поточної мови та контексту існують, вони використовуватимуться. Якщо ні, то Jomres повернеться до мовних визначень, які використовуються в його основних мовних файлах.");
...
Ви можете залишити цільову мову, встановлену для вашої поточної мови, і використовувати цю сторінку для перейменування елементів тут, замість того, щоб відвідувати їх окремі сторінки.');
	...
	jr_define('JOMRES_POLICY_OCCUPANCY_LEVELS_INCLUDE_CHILDREN', "Включити дітей у перевірку рівня заповнення?");
	jr_define('JOMRES_POLICY_OCCUPANCY_LEVELS_INCLUDE_CHILDREN_DESC_MRP', "Коли гості здійснюють бронювання, номери мають максимальну кількість гостей, яку вони можуть прийняти. Якщо загальна кількість гостей перевищує кількість гостей, які можуть прийняти вибрані номери, тоді форма бронювання скаже їм, що їм потрібно вибрати більше кімнат.
