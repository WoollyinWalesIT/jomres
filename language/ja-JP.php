...
jr_define('_JOMRES_MICROMANAGE_MULTIPLE_TARIFFS', "ゲスト数ごとに異なる価格を追加する場合は,<em>部屋タイプごとに複数の料金を作成し</ em>,それらの料金に異なる最小/最大ゲスト値を設定できます。 ");
	...
	jr_define('_JOMRES_MICROMANAGE_INFO', "タリフに名前を付け、タリフが提供される前に予約フォームに必要な最大宿泊数と最小および最大宿泊人数を定義します。<br/><br/>使用 日付ピッカー パネルを使用して、日付の範囲に価格と最低宿泊数の設定を追加するか、入力を直接編集します. 特定の日付に関税をまったく提供したくない場合は、価格を 0 (ゼロ) に設定したままにします <br/><br/> 日付ごとに異なる最低宿泊日数を設定できるため、フェスティバルや会議の週に長時間の予約が必要な場合は、その期間だけ最低宿泊日数を高く設定できます。<br/ ><br/>1 人あたりの料金 (PPPN) を請求する場合は、[設定] > [プロパティの構成] > [料金と通貨] タブでその設定を有効にすることができます。");
