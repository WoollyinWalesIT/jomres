...
jr_define('JOMRES_TARGET_LANGUAGE', 'ターゲット言語');
	...
	jr_define('_JOMRES_COM_A_GATEWAYLIST_INFO', 'このページでは、デポジットの支払いを受け取る支払いゲートウェイを設定できます。支払いゲートウェイのアカウントが必要です。設定ページには、ゲートウェイの設定方法が記載されています。' );

	jr_define('_JOMRES_COM_A_GATEWAYLIST_TESTINGINFO', "スーパー プロパティ マネージャーとしてログインしています。通常、プロパティ マネージャーは支払いゲートウェイを使用できませんが、[管理] > [Jomres] > [設定] > [デバッグ] タブでサイトを開発モードに設定すると、処理できるようになります。 予約を介して、ゲストとして支払いゲートウェイにアクセスします. サイトを開発モードに設定したくない場合は、ログアウトするか、通常の登録ユーザーとしてログインして、ゲストとして予約をテストする必要があります. 個々の支払いゲートウェイのテスト モード設定は、サイトの運用または開発設定の影響を受けません。");