...
jr_define('_JOMRES_EMAIL_TEMPLATES_INSTRUCTIONS', 'Consultare questa pagina per assistenza nella personalizzazione delle e-mail e un elenco dell output disponibile: <a href="http://www.jomres.net/manual/property-managers-guide/48-your -toolbar/settings/254-email-templates" target="_blank">Guida ai modelli di posta elettronica</a>');
...
jr_define('_JOMRES_CONFIG_GMAP_KEY_WARNING', "Non hai un set di chiavi di Google Maps. A causa delle recenti modifiche a Google Maps, tutti i nuovi siti avranno bisogno di una chiave API per poter utilizzare le funzioni di Google Maps. Per favore <a href='http://www.jomres.net/manual/site-managers-guide/14-getting-started/338-google-maps-api-key' target='_blank'>consulta la nostra pagina del manuale su come creare una chiave API< /a> e salva la chiave in Jomres.");
...
jr_define('_JOMRES_FAQ_ADMIN_CATEGORY_INTRODUCTION', 'Introduzione');
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_INTRODUCTION_WHATISJOMRES', 'Cos\'è Jomres?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_INTRODUCTION_WHATISJOMRES', "Jomres è un sistema di prenotazione di hotel e ville che funziona con i sistemi di gestione dei contenuti Joomla e Wordpress. È un sistema di prenotazione completo, pensa 'booking.com' o 'airbnb' senza spese. È un toolkit completo che ti consente di creare un servizio di prenotazione online unico per tutte le strutture che desideri.");
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_INTRODUCTION_USERSADD', 'Gli utenti possono registrarsi sul mio sito Jomres e aggiungere le proprie proprietà?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_INTRODUCTION_USERSADD', 'Sì. Per impostazione predefinita Jomres consente agli utenti registrati di aggiungere le proprie proprietà al tuo sito. Puoi disabilitarlo nella scheda Configurazione sito > Varie se necessario.');
jr_define('_JOMRES_FAQ_ADMIN_CATEGORY_PROPERTIES', 'Proprietà');
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_ADDPROPERTIES', 'Come faccio ad aggiungere una nuova proprietà?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_ADDPROPERTIES', "Quando Jomres è installato, l'utente amministratore viene creato come Super Property Manager. Accedi al frontend del tuo CMS (le pagine pubbliche) come utente amministratore e da lì puoi aggiungere nuove proprietà. <br/ >È possibile creare nuovi gestori nella pagina Gestori proprietà se si desidera aggiungere un nuovo gestore, tuttavia per impostazione predefinita (è possibile disabilitarlo) qualsiasi utente registrato che crea una proprietà viene automaticamente creato come gestore di proprietà per le proprietà che creano. motivi di sicurezza non possono amministrare le proprietà di altri gestori. Questo può essere modificato elevando quell'utente a Super Property Manager nella pagina Property Manager.");
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_IMPORTPROPERTIES', 'Posso importare proprietà da un altro sistema?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_IMPORTPROPERTIES', "Esistono diversi modi per importare proprietà. Il plug-in di importazione proprietà consente di importare proprietà tramite CSV. Ciò consente di importare più proprietà contemporaneamente, ma è una funzionalità riservata all'amministratore. Se la proprietà manager ha un account Beds24, quindi può importare qualsiasi proprietà esistente dal proprio account lì dopo aver collegato le chiavi API.Un altro modo è utilizzare l'API, c'è una funzione per aggiungere e rimuovere proprietà, tuttavia ciò richiederebbe che tu sia uno sviluppatore familiarità con l'utilizzo delle API REST per utilizzare questa funzionalità.");
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_IMPORTBOOKINGS', 'Posso importare prenotazioni da un altro sistema?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_IMPORTBOOKINGS', "Le prenotazioni possono essere importate in una proprietà da un gestore di proprietà tramite il plugin iCal, che offre una funzionalità per importare le prenotazioni nel menu del gestore di proprietà. Inoltre, se un gestore di proprietà ha un account Beds24 e la sua proprietà ha sono stati collegati a Beds24, quindi possono eseguire un'azione Reinvia notifica in Beds24. ");
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_EMAILTEMPLATES', 'Posso modificare i modelli di email?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_EMAILTEMPLATES', 'I gestori di proprietà possono modificare i modelli di email tramite la loro barra degli strumenti in Impostazioni > Modelli di email.');
jr_define('_JOMRES_FAQ_ADMIN_CATEGORY_PAYMENTS', 'Pagamenti');
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_TAKEPAYMENTS', 'Come posso consentire alle proprietà di accettare pagamenti?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_TAKEPAYMENTS', 'Dovrai installare un gateway di pagamento. Jomres ha due gateway di pagamento, che si collegano a Paypal o Stripe. Offre anche un paio di gateway offline e puoi acquistare altri gateway di pagamento dai nostri partner.' );
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_WHICHGATEWAY', 'Quale gateway dovrei usare?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_WHICHGATEWAY', "Se vuoi prelevare una percentuale del deposito di un manager per le tue spese, usa il gateway Stripe. In caso contrario, e intendi invece utilizzare la funzionalità di abbonamento e/o commissione, allora dovrò utilizzare il gateway di Paypal.");
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_PAYMENTACCOUNTS', 'Quando un ospite effettua un pagamento per una prenotazione, chi riceve i soldi?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_PAYMENTACCOUNTS', "Dipende da come hai impostato i gateway di pagamento sul tuo sito. Se stai utilizzando Paypal, puoi configurarlo per sovrascrivere tutte le impostazioni del frontend, essenzialmente rendendo il tuo account l'unico account che riceve pagamenti. Se non scegli di farlo, allora ogni proprietà dovrà configurare il proprio account gateway di pagamento in Impostazioni > scheda Gateways.Se stai utilizzando Stripe, una volta che il gestore della proprietà ha collegato il proprio account al tuo (vedi la documentazione di Stripe in il manuale per maggiori informazioni ) allora riceveranno tutti i fondi, meno una commissione che puoi configurare se scegli di farlo.");
jr_define('_JOMRES_FAQ_ADMIN_CATEGORY_TROUBLESHOOTING', 'Risoluzione dei problemi');
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_TROUBLESHOOTING_EMAIL', "Le email di prenotazione non vengono inviate.");
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_TROUBLESHOOTING_EMAIL', "Se hai problemi a inviare e-mail dal sistema, controlla le impostazioni e-mail del CMS host. Jomres utilizza le impostazioni di Wordpress o Joomla per l'invio di e-mail. Se sono errate, Jomres non sarà in grado di inviare e-mail.");
...
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_TROUBLESHOOTING_NOGATEWAY', "Se hai effettuato l'accesso come Property Manager, non vedrai il gateway di pagamento, perché non paghi te stesso, sciocco. Solo gli utenti non manager vedranno il gateway quando effettuano le prenotazioni.") ;
jr_define('_JOMRES_FAQ_ADMIN_CATEGORY_PURCHASINGJOMRES', 'Licenza Jomres');
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_PURCHASINGJOMRES_FORCEDTOSUBSCRIBE', 'Dopo aver acquistato la licenza X, sono obbligato ad acquistare i rinnovi una volta scaduta la licenza?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_PURCHASINGJOMRES_FORCEDTOSUBSCRIBE', "No, se hai acquistato una licenza Starter, Business o Enterprise, puoi continuare a utilizzare il software a tempo indeterminato, non verrà magicamente compresso. L'eccezione a questa regola è se acquisti un abbonamento. Una volta annullato un abbonamento non potrai più utilizzare i plugin e dovrai disinstallarli.");
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_PURCHASINGJOMRES_EXPIRED', 'La mia licenza è scaduta, posso comunque aggiornare Jomres?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_PURCHASINGJOMRES_EXPIRED', "Anche se fisicamente puoi farlo, non è consigliabile farlo. Una volta scaduta la licenza, non potrai più beneficiare del supporto o scaricare aggiornamenti ai plugin per la tua installazione di Jomres. Ciò significa che dovresti < i>non</i> aggiorna la tua installazione di Jomres dopo che la licenza è scaduta.Questo perché i plugin devono essere aggiornati regolarmente per funzionare con le versioni correnti del sistema, quindi spesso i tuoi plugin più vecchi non funzioneranno su una versione più recente del sistema Se desideri comunque eseguire l'aggiornamento per sfruttare le nuove funzionalità, dovresti acquistare una licenza di rinnovo, che è effettivamente una licenza normale con uno sconto del 50%.");
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_PURCHASINGJOMRES_SOFTWARELICENSE', 'Con quale licenza software è rilasciato Jomres?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_PURCHASINGJOMRES_SOFTWARELICENSE', "Jomres è concesso in licenza con entrambe le licenze MIT e GPL. I plug-in scaricati dalle licenze Starter, Business ed Enterprise sono rilasciati con la stessa licenza e sono completamente gratuiti e open source. I plug-in scaricati per le licenze di prova e in abbonamento sono non sono rilasciati sotto le licenze GPL o MIT e sono codificati Ioncube. Questo approccio ci consente di offrirti una varietà di licenze diverse per soddisfare le tasche di tutti.");
...
jr_define('_JOMRES_FAQ_MANAGER_ANSWER_PAYMENTS', "Per accettare pagamenti online, devi avere un account con un fornitore di servizi di pagamento online, chiamato Gateway. Per vedere i gateway disponibili, vai su Configurazione proprietà > scheda Gateway. Fai clic sul nome di un gateway da prendere alla sua pagina di configurazione.");jr_define('_JOMRES_FAQ_MANAGER_QUESTION_DISCOUNTS', 'Posso fare sconti?');
jr_define('_JOMRES_FAQ_MANAGER_ANSWER_DISCOUNTS', "È possibile concedere sconti, esistono diversi modi per farlo. Se si effettua una prenotazione per conto di un cliente, è possibile impostare il proprio deposito e i totali della prenotazione nel modulo di prenotazione , utilizzando i campi \"Ignora totale alloggio\" e \"Ignora deposito\" ( gli ospiti non possono utilizzare questa funzione ). Un altro modo per offrire uno sconto a un ospite è creare buoni sconto, che possono essere configurati in modo che possano essere utilizzato solo entro determinate date ( Valido dal/al ) o applicato solo quando la prenotazione cade entro determinate date ( Prenotazione valida dal/al ) Questi buoni sconto possono essere assegnati ad un solo ospite, oppure se vuoi puoi stampare il coupon. La stampa include un codice QR che gli ospiti possono scansionare sui loro telefoni che li porta al modulo di prenotazione con il codice sconto già applicato. ");
...
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_CURRENCIES_SETUP', 'Come posso impostare i simboli di valuta?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_CURRENCIES_SETUP', "No. Tutti i simboli di valuta sono già memorizzati nel sistema, e sta ai gestori delle proprietà selezionare la valuta giusta per loro nella pagina Configurazione Proprietà.");
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_CURRENCIES', 'Come abilito più valute?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_CURRENCIES', "Jomres ha la funzionalità per convertire i prezzi da una valuta all'altra automaticamente. Per prima cosa devi visitare la scheda Configurazione sito > Conversione valuta. Lì troverai i collegamenti a due servizi online. Il primo servizio, una volta L'API è stata salvata, consente a Jomres di scaricare i dati di conversione della valuta una volta al giorno. Il secondo abilita la geolocalizzazione basata su IP per gli utenti. Richiedi una chiave API gratuita da ciascun servizio. Con questi due servizi combinati, Jomres può rilevare la valuta locale dell'utente e visualizzare i prezzi delle proprietà sia nella valuta locale dell'ospite, sia nella valuta dell'hotel. Se non imposti queste chiavi API, Jomres utilizzerà per impostazione predefinita il simbolo dell'euro. Se lo desideri, puoi impostare un codice di valuta globale. Ciò significa che la proprietà i gestori non saranno in grado di selezionare altre valute, tuttavia i potenziali ospiti continueranno a vedere i prezzi nella propria valuta locale a meno che non abbiano utilizzato la funzione di cambio valuta per cambiare la valuta in cui sono visualizzati i prezzi. ");
jr_define('_JOMRES_FAQ_MANAGER_QUESTION_LANGUAGES', 'Come faccio a salvare le descrizioni in più lingue?');
jr_define('_JOMRES_FAQ_MANAGER_ANSWER_LANGUAGES', "Per salvare le descrizioni in più lingue, visita prima la pagina Impostazioni > Dettagli proprietà. Salva lì la descrizione della tua proprietà. Successivamente, cambia la lingua in cui stai visualizzando il sito. Ora vai alle Impostazioni Di nuovo la pagina Dettagli proprietà e salva di nuovo i dettagli. Quindi, se il sito è in grado di mostrare sia l'inglese che lo spagnolo (o qualsiasi altra lingua), dovresti selezionare Inglese, inserire la descrizione in inglese, quindi fare clic su Salva. Quindi, cambia il tuo attuale lingua in spagnolo, quindi salva la nuova descrizione spagnola. Funziona per tutti gli input in quella pagina.");
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_PROPERTIES_NUMBER', 'Quante proprietà posso elencare?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_PROPERTIES_NUMBER', "Non esiste un limite codificato al numero di proprietà che puoi elencare, l'unica limitazione è nella limitazione fisica del server stesso. Se si tratta di un server con molti altri account attivi, le risorse saranno limitate, se , d'altra parte è un server dedicato o virtuale dedicato, quindi potrai elencare molte più proprietà.");
...
jr_define('_JOMRES_FAQ_MANAGER_ANSWER_GUESTTYPES', "In Impostazioni > Configurazione proprietà > scheda Tariffe e valute, puoi decidere se vuoi addebitare per persona per notte. Se addebiti per persona per notte, dovrai creare uno o più tipi di ospite. Puoi creare un semplice tipo di ospite, dove devi solo dare loro una descrizione ( ad esempio 'Persone' ), oppure puoi creare diversi tipi di ospite, ad esempio 'Adulti' e 'Bambini sotto i 10'. Per i bambini, se lo desideri per offrire uno sconto del 50%, allora imposteresti 'È percentuale' su Sì e il valore Varianza su 50. Le camere hanno tariffe di base, questa impostazione ti consente di regolare il prezzo della camera in base al tipo di ospite.") ;
...
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_INTRODUCTION_PROFILES', 'Posso trasformare un utente in un gestore di proprietà?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_INTRODUCTION_PROFILES', "Sì puoi, tramite la pagina Gestori di proprietà in Gestione utenti. Se l'utente è già registrato, nell'input nella parte superiore della pagina Gestori di proprietà digita i primi caratteri del suo nome utente e il suo nome utente sarà appaiono sotto. Fai clic sul loro nome utente e verrai indirizzato a una pagina in cui puoi configurare a quali proprietà hanno accesso. Se non sono già registrati, dovrai prima utilizzare le pagine di gestione degli utenti del tuo CMS host per aggiungerli come utente del CMS.<br/><br/> Un consiglio: se intendi addebitare ai gestori di proprietà l'elenco delle loro proprietà sul tuo sito, tramite la ricezione di commissioni tramite il gateway Stripe o la funzionalità di abbonamento/commissione, noi non consigliamo di aggiungere utenti in questo modo. In questo caso, solo i gestori di proprietà super dovrebbero essere aggiunti utilizzando questo processo e quando vengono aggiunti non dovresti selezionare alcuna proprietà nell'elenco sottostante. Gli utenti dovrebbero essere incoraggiati a creare da soli le proprietà nel frontend. Questo li aggiungerà automaticamente come gestori di proprietà e assicurerà che venga fatturato l'utente corretto.");
jr_define('_JOMRES_FAQ_ADMIN_CATEGORY_PLUGINS', 'Plugin');
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_GENERAL_PLUGINS', 'A cosa serve il Plugin Manager?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_GENERAL_PLUGINS', 'Il Plugin Manager è uno strumento chiave in Jomres. Ci permette di distribuire la versione più aggiornata dei plugin Jomres attraverso una funzione di download e installazione automatica.');
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_GENERAL_PLUGINS_IONCUBE', 'Perché vedo un avviso su Ioncube Loader quando clicco sulla pagina Plugin Manager?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_GENERAL_PLUGINS_IONCUBE', "Hai salvato una chiave di licenza di prova nella configurazione del sito, ma il tuo server web non offre i caricatori Ioncube più recenti. Chiedi ai tuoi host di installare i caricatori, che possono essere trovati <a href='http ://www.ioncube.com/loaders.php' target='_blank'>su questa pagina.</a> Se hai acquistato una licenza completa senza abbonamento e pensi di vederla per errore, potrebbe aver dimenticato di inserire la chiave di licenza completa nella pagina Configurazione del sito.");
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_GENERAL_PLUGINS_INSTALLATION', 'Come installo un plugin?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_GENERAL_PLUGINS_INSTALLATION', "Supponendo che tu abbia un numero di chiave di licenza valido salvato nella configurazione del sito, tutto ciò che devi fare è fare clic sul pulsante Installa accanto al nome del plug-in nel gestore dei plug-in. Jomres installerà il plug-in per te e porterà in seguito torni alla pagina dei plugin. Occasionalmente, invece, potresti essere indirizzato a una pagina di scoperta se stai eseguendo Jomres in Joomla. Basta fare clic sul pulsante Scopri per installare il plugin.<br/><br/>NOTA Don' t installate tutti i plug-in, non funzionano tutti insieme (in base alla progettazione). Potresti ritrovarti con pagine vuote nel modulo di prenotazione. Invece, identifica i plug-in di cui hai bisogno e sperimentali.");
jr_define('_JOMRES_FAQ_ADMIN_CATEGORY_SITESTRUCTURE', 'Struttura del sito');
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_SITESTRUCTURE_INTRO', 'Cosa significa struttura del sito?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_SITESTRUCTURE_INTRO', "La struttura del sito in Jomres si riferisce ai vari elementi del tuo sito che lo rendono unico. Definisce i tipi di proprietà che elenchi (ad es. ville o hotel) e i tipi di camere/sottotipi per quei tipi di proprietà. Quindi , se vendi vacanze con uso cucina, vendi affitti di qualcosa come uno chalet o una villa. Chalet diversi potrebbero avere un numero diverso di camere, balconi, ecc. Nel caso di hotel o B&B, vendi stanze <em>nella</em> della proprietà e sarebbero camere doppie, singole ecc. Per chiarire questo aspetto, un'installazione predefinita ha già diverse stanze e tipi di proprietà creati per aiutarti a capire. Tours, non provare a creare un nuovo tipo di proprietà senza prima leggere la sezione su Tours.");
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_SITESTRUCTURE_PROPERTYTYPES', 'Cosa sono i tipi di proprietà?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_SITESTRUCTURE_PROPERTYTYPES', 'In questo sistema, un tipo di proprietà si riferisce, letteralmente, al tipo di proprietà. Può essere un hotel, una villa, un appartamento. I gestori di proprietà non possono creare tipi di proprietà perché i tipi di proprietà sono un elemento che può essere cercato dagli ospiti. Per garantire che la tua funzionalità di ricerca rimanga pulita e ordinata, solo gli amministratori del sito possono creare tipi di proprietà.');
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_SITESTRUCTURE_PROPERTYFEATURES', 'Cosa sono le caratteristiche della proprietà?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_SITESTRUCTURE_PROPERTYFEATURES', "Le caratteristiche della proprietà sono cose che rendono una proprietà unica. Ad esempio, una caratteristica della proprietà potrebbe essere che è \"Vicino alla spiaggia\" mentre un altro hotel potrebbe avere la caratteristica di essere all'interno di un parco nazionale, offre percorsi a cavallo, qualcosa del genere. Come i tipi di proprietà, i gestori non possono creare nuove funzionalità, possono solo selezionare da quelle che crei. Le funzionalità della proprietà possono avere immagini assegnate a loro. Innanzitutto è necessario caricare le immagini delle funzionalità tramite la struttura del sito > Media Center > Caratteristica della proprietà Icone a discesa. Carica tutte le icone di funzionalità che desideri, quindi quando crei una funzionalità di proprietà puoi sceglierne l'immagine.");
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_SITESTRUCTURE_PROPERTYFEATURECATEGORIES', 'Cosa sono le categorie di caratteristiche delle proprietà?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_SITESTRUCTURE_PROPERTYFEATURECATEGORIES', "Le caratteristiche della proprietà sono visualizzate nella pagina dei dettagli della proprietà, tra le altre. Nella pagina dei dettagli della proprietà, le caratteristiche possono essere visualizzate raggruppate per categorie, ad esempio 'Area soggiorno', 'Accessibilità' ecc.");
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_SITESTRUCTURE_ROOMFEATURES', 'Quali sono le caratteristiche della stanza?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_SITESTRUCTURE_ROOMFEATURES', "I gestori di proprietà che utilizzano la modalità di modifica delle tariffe Microgestione o Avanzata sono in grado di modificare direttamente le loro stanze. Possono anche creare cose chiamate Caratteristiche della stanza che vengono visualizzate nella pagina di disponibilità della stanza per ogni proprietà. Se utilizzare la visualizzazione dell'elenco delle camere Classic nel modulo di prenotazione, quindi gli ospiti possono utilizzare le funzionalità della camera per filtrare le camere prima di selezionarne una o più. Nella pagina Struttura del sito > Caratteristiche della camera I gestori del sito sono anche in grado di creare caratteristiche della camera e queste funzionalità può essere visualizzato in tutti i moduli di prenotazione della struttura in cui la struttura utilizza l'elenco delle camere classiche. Fondamentalmente, è una funzione della camera globale. Queste caratteristiche della camera possono essere configurate per essere mostrate solo a determinati tipi di proprietà e le immagini per queste caratteristiche della camera possono essere caricate tramite la pagina Struttura del sito > Media Center.");
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_SITESTRUCTURE_COUNTRIES', 'Perché esiste una pagina per modificare paesi/regioni?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_SITESTRUCTURE_COUNTRIES', "Penseresti che un elenco di paesi fosse abbastanza immutabile, ma ti sbaglieresti. Nel corso degli anni, quando manteniamo un elenco di paesi, abbiamo visto molti cambiamenti. Di conseguenza, noi appreso che è più facile consentire ai gestori del sito di definire i paesi che desiderano mostrare sul sito. Ciò consente al catalano irriducibile di visualizzare la propria provincia come un paese unico, se questa è l'affermazione che desiderano fare. Lo stesso vale per altre regioni del mondo.");
jr_define('_JOMRES_FAQ_ADMIN_CATEGORY_LOCALISATION', 'Localizzazione/Traduzione');
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_LOCALISATION_INTRO', 'Come faccio a tradurre le cose?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_LOCALISATION_INTRO', "La traduzione di etichette/stringhe viene eseguita tramite le opzioni del menu Lingue, principalmente le pagine Traduci file di lingua/Traduci locali. Poiché i file di lingua sono molto grandi, quando fai clic su questa pagina possono impiegare un po' di tempo per caricamento. <br/><br/>Facciamo un esempio. Supponiamo che tu voglia tradurre la Germania in Deutschland. Prima fai clic su Traduci locali. Quindi controlla il menu a discesa della lingua in alto a destra dall'inglese al tedesco. Quindi, scorri in basso nella pagina fino ad arrivare in Germania. Puoi vedere che ha una sottolineatura tratteggiata. Ciò significa che puoi tradurlo. Fai clic sulla parola, vedrai un popup contenente la parola \"Germania\". Cambialo in \"Deutschland\" e fai clic il segno di spunta verde. Se il tuo CMS host è configurato per offrire pagine in lingua tedesca ai visitatori del sito, la parola \"Germania\" verrà ora mostrata come \"Germania\". Questo concetto si applica in tutta Jomres. <br/><br/> Nelle pagine del Gestore della proprietà, anche i Gestori della proprietà potrebbero vedere un menu a discesa Modalità di modifica delle etichette nella parte superiore delle loro pagine. Quando questo è impostato su On, saranno anche in grado di apportare modifiche al frontend, tuttavia tali modifiche influenzeranno solo la loro proprietà attualmente attiva, quindi ad esempio possono facilmente cambiare la parola Tariffe in Prezzi per una sola proprietà se è quello che è necessario.");
jr_define('_JOMRES_FAQ_ADMIN_CATEGORY_TOURS', 'Tour');
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_TOURS_INTRO', 'Cosa sono i tour?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_TOURS_INTRO', "Può sembrare una domanda stupida, ma in realtà non lo è. Nel corso degli anni abbiamo imparato che culture diverse hanno percezioni diverse di cosa sia un \"tour\". Di conseguenza, le aspettative di vendita di tour variano notevolmente, quindi prima di discutere dei tour, definiremo prima cosa pensiamo dei tour. <br/> Un tour è un semplice viaggio o visita per vedere un'attrazione o un monumento. Tuttavia, un soggiorno in un hotel non è un tour è possibile partecipare ai tour durante il soggiorno in hotel.");
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_TOURS_HOWTOSELL', 'Come uso Jomres per vendere tour?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_TOURS_HOWTOSELL', 'Per affrontare la questione della vendita di tour a Jomres, devi prima installare il plug-in Jintour. Questo plug-in ti consente di creare tour che vengono venduti come parte di case vacanza. Ti consente anche di creare proprietà che sono esclusivamente per la vendita di tour. Una volta installato questo plugin, i gestori di proprietà saranno in grado di creare i tour e le proprietà dei tour nel frontend.');
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_TOURS_WHATISJINTOUR', 'Cos\'è il plugin Jintour?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_TOURS_WHATISJINTOUR', 'Jintour sta per Jomres INtegrated TOURs. Ti consente di creare risorse semplici e limitate che possono essere prenotate, che non sono stanze in una proprietà. Sebbene il plugin sia progettato principalmente per consentire la prenotazione di tour, essi possono essere utilizzati per molte cose, non solo per i tour. Possono essere utilizzati, ad esempio, da una baita che vuole noleggiare sci e attrezzatura. Poiché questa attrezzatura avrà una disponibilità limitata, è possibile utilizzare il plugin Jintour per consentire agli ospiti di prenota e paga questa attrezzatura al momento della prenotazione.');
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_TOURS_SAMEBOOKING', 'Come posso vendere tour e case vacanza nella stessa prenotazione?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_TOURS_SAMEBOOKING', "In questo scenario, hai un hotel e vuoi vendere tour a un'attrazione locale. Accedi al frontend e vai all'hotel che hai creato. Quindi, fai clic su Impostazioni> Tour /Gestione attività. In questa pagina, segui le istruzioni sullo schermo per creare prima un profilo del tour, quindi generare i tour. Una volta fatto, sarai in grado di vendere sia i tour che le prenotazioni delle camere e gli extra opzionali insieme nello stesso modulo di prenotazione .");
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_TOURS_STANDALONETOURS', 'E se volessi vendere solo tour?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_TOURS_STANDALONETOURS', "Per prima cosa dovrai creare una nuova proprietà nel frontend. Quando lo fai, assicurati di impostare il 'Tipo di proprietà' su Tour. Quando avrai creato questa nuova proprietà, potrai per creare nuovamente i tour attraverso la pagina Impostazioni > Gestione Tour/Attività.Potrai anche offrire extra opzionali come nelle normali strutture, l'unica cosa che non potrai fare è configurare le stanze, le caratteristiche delle stanze o le tariffe come vuoi non vendiamo stanze in questa proprietà.");
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_TOURS_GLOBALTOURS', 'Come posso creare una risorsa Tour/prenotabile che può essere visualizzata in tutti i moduli di prenotazione delle proprietà?');
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_TOURS_GLOBALTOURS', "Nell'area amministratore del tuo sito, visita la pagina Portale > Tour funzionalità/Gestione attività. Il profilo del tour/processo di generazione è lo stesso utilizzato nel frontend, tuttavia tutti i tour generati qui sono visualizzati nella frontend. Uno scenario tipico in cui questo può essere utilizzato è se l'amministratore del sito sta organizzando eventi in tutto il paese e vuole vendere prenotazioni di hotel. Creerebbero gli hotel partecipanti nel frontend come di consueto, impostando i prezzi delle camere ecc., Quindi il ' tour' in questo caso sarebbe effettivamente la quota di partecipazione all'evento stesso.");
jr_define('_JOMRES_FAQ_ADMIN_QUESTION_INTRODUCTION_FIRSTTHINGSFIRST', "Ok, ho installato Jomres, e poi?");
jr_define('_JOMRES_FAQ_ADMIN_ANSWER_INTRODUCTION_FIRSTTHINGSFIRST', "La primissima cosa che dovresti fare, ora che hai installato Jomres è fare clic su Aiuto nel menu a sinistra, quindi su Per iniziare. Seriamente, vai via e fallo ora.") ;
...
jr_define('VALID_LICENSE_MESSAGE', "Congratulazioni, stai utilizzando un numero di licenza valido e puoi installare i plugin Core tramite il gestore plugin di Jomres.");jr_define('_JOMRES_MEDIA_CENTRE_INSTRUCTIONS_MRP', 'Questa pagina ti consente di caricare immagini per la tua proprietà e le cose che offri. Ha un pulsante principale e un secondo pulsante che ti consente di selezionare risorse specifiche per le quali caricare immagini. Quindi, la proprietà principale image e lo Slideshow mostrerà solo un pulsante ma se hai creato ad es. Extra opzionali o Stanze, vedrai un secondo pulsante dove puoi caricare immagini per quelle risorse specifiche.');
