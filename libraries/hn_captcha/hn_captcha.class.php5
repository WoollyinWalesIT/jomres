...
class hn_captcha
{
    ...
    //
        ...
        public function __construct($config, $debug=FALSE, $secure=TRUE)
        {
            ...
            $this->messages = array(
                 'en'=>array(
                            'msg1'=>jr_gettext('_JOMRES_FRONT_MR_MENU_CAPTCHA_MSG1',_JOMRES_FRONT_MR_MENU_CAPTCHA_MSG1),
                            'msg2'=>jr_gettext('_JOMRES_FRONT_MR_MENU_CAPTCHA_MSG2',_JOMRES_FRONT_MR_MENU_CAPTCHA_MSG2),
                            'buttontext'=>jr_gettext('_JOMRES_FRONT_MR_MENU_CAPTCHA_BUTTONTEXT',_JOMRES_FRONT_MR_MENU_CAPTCHA_BUTTONTEXT,false,false),
    ...
    //
        ...
        public function display_form($only_body=FALSE)
        {
            ...
            $s .= '<p class="captcha_1"><input class="jomres_inputbox" type="text" name="hncaptcha_private_key" value="" maxlength="'.$this->chars.'" size="'.$this->chars.'">';
            if($this->refreshlink)
            {
                $s .= '<p>'.$this->msg2;
                $s .= ' <input class="button" type="submit" name="hncaptcha_refresh" value="'.$this->refreshbuttontext.'">'."</p>\n";
            }
			$s .= '<input type="button" name="send" value="'.$this->buttontext.'" class="button" onclick="validateContactForm();"/>'."</p>\n";
            if(!$only_body)
            {
                $s .= ''."\n";
                $s .= '</div>';
