...
/**
#
 * adodb-time. Included for users on Windows
#
 *
 * @package Jomres
#
...
function adodb_dow( $year, $month, $day )
	...
	*/
	if ( $year <= 1582 )
		{
		if ( $year < 1582 || ( $year == 1582 && ( $month < 10 || ( $month == 10 && $day < 15 ) ) ) ) $greg_correction = 3;
		else
		$greg_correction = 0;
		}
	else
...
function _adodb_getdate( $origd = false, $fast = false, $is_gmt = false )
	...
	if ( $fast )
		{
		return array ( 'seconds' => $secs, 'minutes' => $min, 'hours' => $hour, 'mday' => $day, 'mon' => $month, 'year' => $year, 'yday' => floor( $secsInYear / $_day_power ), 'leap' => $leaf, 'ndays' => $ndays );
		}


	$dow = adodb_dow( $year, $month, $day );

	return array ( 'seconds' => $secs, 'minutes' => $min, 'hours' => $hour, 'mday' => $day, 'wday' => $dow, 'mon' => $month, 'year' => $year, 'yday' => floor( $secsInYear / $_day_power ), 'weekday' => gmdate( 'l', $_day_power * ( 3 + $dow ) ), 'month' => gmdate( 'F', mktime( 0, 0, 0, $month, 2, 1971 ) ), 0 => $origd );
	}
...
function adodb_date2( $fmt, $d = false, $is_gmt = false )
	...
	if ( $d !== false )
		{
		if ( !preg_match( "|^([0-9]{4})[-/\.]?([0-9]{1,2})[-/\.]?([0-9]{1,2})[ -]?(([0-9]{1,2}):?([0-9]{1,2}):?([0-9\.]{1,4}))?|", ( $d ), $rr ) ) return adodb_date( $fmt, false, $is_gmt );
	...
	*/
	for ( $i = 0; $i < $max; $i++ )
		...
		switch ( $fmt[ $i ] )
		{
			...
			// HOUR
			case 'Z':
				$dates .= ( $is_gmt ) ? 0 : -adodb_get_gmt_diff();
				break;
			case 'O':
...
function adodb_mktime( $hr, $min, $sec, $mon = false, $day = false, $year = false, $is_dst = false, $is_gmt = false )
	...
	if ( !defined( 'ADODB_TEST_DATES' ) )
		...
		if ( 1971 < $year && $year < 2038 || !defined( 'ADODB_NO_NEGATIVE_TS' ) && ( 1901 < $year && $year < 2038 )
		)
			{
...
function adodb_strftime( $fmt, $ts = false, $is_gmt = false )
	...
	for ( $i = 0, $max = strlen( $fmt ); $i < $max; $i++ )
		...
		if ( $ch == '%' )
			...
			switch ( $ch )
			{
				...
				case 'u':
					$fmtdate .= '?u';
					$parseu = true;
					break; // wrong strftime=1-based, date=0-basde
				case 'U':
					$fmtdate .= '?U';
					$parseU = true;
		...
		else if ( ( 'A' <= ( $ch ) && ( $ch ) <= 'Z' ) || ( 'a' <= ( $ch ) && ( $ch ) <= 'z' ) ) $fmtdate .= "\\" . $ch;
		else
		$fmtdate .= $ch;
		}
	//echo "fmt=",$fmtdate,"<br>";
