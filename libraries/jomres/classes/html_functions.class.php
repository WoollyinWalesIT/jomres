...
class html_functions
	...
	function cpanelButton( $link, $image, $text, $path = "JOMRES_ROOT_DIRECTORY/images/jomresimages/small/", $category = null, $external = false, $disabled = false )
		{
		...
		if ( !file_exists( JOMRESCONFIG_ABSOLUTE_PATH . JRDS . JOMRES_ROOT_DIRECTORY . JRDS . 'images' . JRDS . $image ) )
			{
			if ( !file_exists( JOMRESCONFIG_ABSOLUTE_PATH . JRDS . JOMRES_ROOT_DIRECTORY . JRDS . 'images' . JRDS . 'jomresimages' . JRDS . 'small' . JRDS . $image ) ) $path = get_showtime( 'eLiveSite' ) . $image;
			else
