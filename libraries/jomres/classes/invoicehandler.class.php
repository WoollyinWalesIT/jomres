...
class invoicehandler extends jrportal_invoice
	...
	function create_new_invoice( $invoice_data, $line_items = array () )
		...
		*/
		...
		if ( !isset( $invoice_data[ 'is_commission' ] ) ) $this->is_commission = 0;
		else
		$this->is_commission = (int) $invoice_data[ 'is_commission' ];

		if ( !isset( $invoice_data[ 'status' ] ) ) $this->status = $default_status;
		else
		$this->status = (int) $invoice_data[ 'status' ];

		$this->raised_date = date( 'Y-m-d H:i:s' );
		if ( !isset( $invoice_data[ 'due_date' ] ) ) $this->due_date = $this->raised_date;
		else
		$this->due_date = $invoice_data[ 'due_date' ];

		if ( !isset( $invoice_data[ 'subscription' ] ) ) $this->subscription = $default_subscription;
		else
		$this->subscription = (int) $invoice_data[ 'subscription' ];

		if ( !isset( $invoice_data[ 'currencycode' ] ) ) $this->currencycode = $default_currencycode;
		else
		$this->currencycode = $invoice_data[ 'currencycode' ];

		if ( !isset( $invoice_data[ 'recur_frequency' ] ) ) $this->recur_frequency = "0";
		else
		$this->recur_frequency = $invoice_data[ 'recur_frequency' ];

		if ( !isset( $invoice_data[ 'recur_dayofmonth' ] ) ) $this->recur_dayofmonth = 0;
		else
	...
	function update_invoice( $invoice_data, $line_items = array () )
		...
		if ( !isset( $invoice_data[ 'due_date' ] ) ) $this->due_date = date( 'Y-m-d H:i:s' );
		else
		$this->due_date = $invoice_data[ 'due_date' ];
		//var_dump($line_items);
		foreach ( $line_items as $item )
	...
	function mark_invoice_paid()
		...
		if ( number_format( $balance, 2, '.', '' ) > 0.00 )
			{
			$line_item_data = array ( 'tax_code_id' => 0, 'name' => jr_gettext( '_JOMRES_AJAXFORM_BILLING_BALANCE_PAYMENT', _JOMRES_AJAXFORM_BILLING_BALANCE_PAYMENT, false, false ), 'description' => '', 'init_price' => "-" . number_format( $balance, 2, '.', '' ), 'init_qty' => "1", 'init_discount' => "0", 'recur_price' => "0.00", 'recur_qty' => "0", 'recur_discount' => "0.00" );
