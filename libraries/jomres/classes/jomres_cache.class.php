...
class jomres_cache
	...
	function setUserID( $userSpecific )
		...
		if ( $userSpecific ) $this->user_id = $thisJRUser->userid;
		else
		$this->user_id = 0;
		}
	...
	function generateFilename()
		...
		if ( $thisJRUser->userIsRegistered ) $registeredFlag = "0";
		if ( $thisJRUser->accesslevel == "1" ) $receptionistFlag = "1";
		if ( $thisJRUser->accesslevel == "2" ) $managerFlag = "1";
		if ( $this->superPropertyManager ) $superPropertyManagerFlag = "1";
		if ( $this->user_id > 0 ) $prefix = md5( $jomresConfig_secret . $this->user_id );
		else
	...
	function readCache()
		{
		if ( $this->useCaching == "0" ) return false;
		if ( !is_dir( $this->cache_folder ) OR !is_writable( $this->cache_folder ) ) return false;

		$cache_path = $this->cache_folder . $this->filename;

		if ( !@file_exists( $cache_path ) ) return false;

		if ( filemtime( $cache_path ) < ( time() - $this->expiration ) ) return false;
		...
		if ( filesize( $cache_path ) > 0 ) $cache = unserialize( fread( $fp, filesize( $cache_path ) ) );
		else
