...
class jomres_reviews
	...
	public function this_user_can_report()
		{
		$thisJRUser = jomres_singleton_abstract::getInstance( 'jr_user' );
		if ( $thisJRUser->userIsRegistered ) return true;
	...
	public function this_user_can_review()
		...
		if ( $jrConfig[ 'reviews_test_mode' ] == "1" )
			{
			if ( $thisJRUser->userIsRegistered ) $can_post = true;
			}
		else
			{
			if ( !$thisJRUser->userIsManager && $thisJRUser->userIsRegistered ) $can_post = true;
			}
	...
	public function this_user_can_review_this_property()
		...
		$thisJRUser = jomres_singleton_abstract::getInstance( 'jr_user' );
		// This test mode allows property managers to make reviews where normally they wouldn't be allowed to.
		if ( $jrConfig[ 'reviews_test_mode' ] == "1" && $thisJRUser->userIsRegistered ) return true;
		if ( $this->property_uid == 0 ) return false;
		if ( !$thisJRUser->userIsRegistered ) return false;
		if ( $thisJRUser->userIsManager ) return false;
	...
	public function save_review( $rating, $title, $description, $pros, $cons )
		...
		$siteConfig = jomres_singleton_abstract::getInstance( 'jomres_config_site_singleton' );
		$jrConfig   = $siteConfig->get();
		if ( $jrConfig[ 'autopublish_reviews' ] == "1" ) $published = 1;
	...
	public function getRatingsMulti( $property_uids )
		{
		if ( !isset( $this->multi_query_result ) ) $this->multi_query_result = array ();
		$sql    = "SELECT item_id, AVG(rating) as avg_rating, COUNT(rating) as counter, SUM(rating) as sumrating FROM #__jomres_reviews_ratings WHERE item_id IN (" . implode(',',$property_uids) . ") AND published = 1 GROUP BY item_id";
		$result = doSelectSql( $sql );
		$arr    = array ();
		if ( count( $result ) > 0 )
			{
		...
		foreach ( $property_uids as $property_uid )
			{
			if ( isset( $arr[ $property_uid ] ) ) $this->multi_query_result[ $property_uid ] = $arr[ $property_uid ];
			else
				{
	...
	public function showConfirm( $rating_id )
		...
		foreach ( $result as $res )
			...
			if ( (int) $res->confirm == 1 ) $arr[ 'agree' ]++;
			else
			$arr[ 'disagree' ]++;
			}
