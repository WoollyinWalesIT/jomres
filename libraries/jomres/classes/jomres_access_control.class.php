...
class jomres_access_control
	...
	public function __construct()
		...
		if ( jomres_cmsspecific_areweinadminarea() ) $this->feature_enabled = false;
		else
		...
		if ( $jrConfig[ 'full_access_control' ] == "1" ) $this->limit_to_menus_only = false;
	...
	function remove_minicomp_from_access_control_table( $minicomp )
		{
		$query = "DELETE FROM #__jomres_access_control WHERE scriptname = '" . (string) $minicomp . "' LIMIT 1 ";
		if ( !doInsertSql( $query, "" ) ) trigger_error( "Unable to delete from access control table, mysql db failure", E_USER_ERROR );
		}
	...
	function can_user_access_this_script( $minicomp )
		...
		if ( in_array( $minicomp, $this->uncontrollable_scripts ) ) return true;
		if ( in_array( $pattern, $this->uncontrollable_patterns ) ) return true;
		...
		if ( !is_null( $thisJRUser ) )
			...
			if ( !$this->feature_enabled ) return true;
			else
				{
					...
					if ( array_key_exists( $minicomp, $this->controlled ) )
						...
						switch ( $this->controlled[ $minicomp ][ 'access_level' ] )
						{
							...
							case "anybody":
								...
								break;
							case "registered":
								if ( $users_access_level == "supermanager" || $users_access_level == "manager" || $users_access_level == "reception" || $users_access_level == "registered" ) $user_can_access = true;
								break;
							case "reception":
								if ( $users_access_level == "supermanager" || $users_access_level == "manager" || $users_access_level == "reception" ) $user_can_access = true;
								break;
							case "manager":
								if ( $users_access_level == "supermanager" || $users_access_level == "manager" ) $user_can_access = true;
								break;
							case "supermanager":
								if ( $users_access_level == "supermanager" ) $user_can_access = true;
								break;
							case "nobody":
							default :
								$user_can_access = false;
	...
	function recount_controlled_scripts()
		{
		$this->init();
		}
	...
	function generate_access_control_dropdown( $minicomp )
		...
		if ( count( $this->controlled ) > 0 )
			...
			if ( array_key_exists( $minicomp[ 'eventPoint' ] . $minicomp[ 'eventName' ], $this->controlled ) ) $current_level = $this->controlled[ $minicomp[ 'eventPoint' ] . $minicomp[ 'eventName' ] ][ 'access_level' ];
			else
		...
		$mode_options = array ();
		if ( $current_level == "default" ) $mode_options[ ] = jomresHTML::makeOption( 'default', $this->levels[ 'default' ] );
