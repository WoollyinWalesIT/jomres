...
function admins_first_run($manual_trigger = false)
	...
	if (!file_exists($logfile) || $manual_trigger)
		...
		<ol>
			...
			<li>The last thing you need to do in the administrator area is to go to the <a href="'.get_showtime('live_site').'/administrator/index.php?option=com_modules" target="_blank">Joomla Module Manager</a> and add a login module to the frontend, if you haven\'t already got one.</li>
			<li>Now go to the <a href="'.get_showtime('live_site').'/index.php" target="_blank">public pages</a> of your site and log in as "admin". When you click on the <a href="'.get_showtime('live_site').'/index.php?option=com_jomres" target="_blank">Main Menu link to Jomres</a>, as you are logged in, you will see the <a href="http://manual.jomres.net/property_managers_guide_toolbar.html" target="_blank">Property Manager\'s toolbar</a>. If you hover over each button you will see some text that changes depending on the button you are hovering over. Go to the button marked "Tariffs & Rooms" and click on it. It\'s on the second row, second from the left.</li>
		...
		if ($count == $threshold)
			...
			echo '<p style="font-size:80%">This is the one and only time you will see this feedback request (unless you delete <i>"'.$logfile.'"</i>).</p>';
			echo '</div>';
...
function hotelSettings()
	...
	$pricingOutput=array();
	if ($mrConfig['wholeday_booking'] == "1")
		$pricingOutput[]= jomresHTML::makeOption( 'D', _JOMRES_BOOKINGFORM_PRICINGOUTPUT_DAILY_WHOLEDAY );
	else
