...
function jr_get_defined( $constant, $default = '' )
    ...
    if ( !defined( $constant ) )
        ...
        if ( is_null( $result ) && $default != '' ) $result = $default;
...
function jr_gettext( $theConstant, $theValue, $okToEdit = true, $isLink = false )
    ...
    global $text_bucket;
    if ( !isset( $text_bucket ) ) $text_bucket = array ();
    if ( isset( $text_bucket[ $property_uid ][ $theConstant ][ $okToEdit ] ) ) return $text_bucket[ $property_uid ][ $theConstant ][ $okToEdit ];
    ...
    if ( get_showtime( "jr_user_ready" ) ) // If jr_user isn't ready yet, calling jomres_singleton_abstract::getInstance('jr_user') will cause php to stop due to recursion, so we'll check that jr_user's been set up before we do anything else
        ...
        if ( !$thisJRUser->userIsManager ) $tmpBookingHandler->user_settings[ 'editing_on' ] = false;

        if ( $thisJRUser->userIsManager && $thisJRUser->accesslevel < 2 ) $tmpBookingHandler->user_settings[ 'editing_on' ] = false;
    ...
    if ( isset( $thisJRUser->accesslevel ) ) $accessLevel = $thisJRUser->accesslevel;
    else
    $accessLevel = 0;

    $br = "";
    if ( get_showtime( 'task' ) == "editCustomTextAll" ) $br = "<br>";
    ...
    if ( isset( $thisJRUser ) )
        ...
        if ( $jrConfig[ 'allowHTMLeditor' ] != "1" ) $theText = jomres_remove_HTML( $theText );
        ...
        if ( $thisJRUser->userIsManager && ( $editing || ( $jrConfig[ 'editingModeAffectsAllProperties' ] == "1" && $thisJRUser->superPropertyManager == true ) ) && $okToEdit && ( $accessLevel == 2 ) )
            {
            if ( strlen( trim( $theText ) ) == 0 || strtolower( trim( $theText ) ) == "<span></span>" || strtolower( trim( $theText ) ) == "<span> </span>" || strtolower( trim( $theText ) ) == "<span>  </span>" ) $theText = "";
            $indexphp = "index.php";
