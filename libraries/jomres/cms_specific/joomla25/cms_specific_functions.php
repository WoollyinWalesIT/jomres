...
function jomres_cmsspecific_areweinadminarea()
	...
	if ( strpos( $_SERVER[ 'SCRIPT_NAME' ], "administrator" ) ) $administrator_area = true;
	...
	if ( !$thisJRUser->userIsRegistered )
		...
		if ( !$id )
			...
			if ( $jrConfig[ 'useNewusers_sendemail' ] == "1" )
				{
				if ( !jomresMailer( get_showtime('mailfrom'), get_showtime('fromname'), $guestDeets[ 'email' ], $subject, $text, $mode = 0 ) ) error_logging( 'Failure in sending registration email to guest. Target address: ' . $hotelemail . ' Subject' . $subject );
				}
			}
		}
...
function jomres_cmsspecific_stringURLSafe( $str )
	...
	if ( !strstr( $scriptname, 'install_jomres.php' ) )
		...
		if ( $config->getValue( 'config.unicodeslugs' ) == '1' ) $str = JFilterOutput::stringURLUnicodeSlug( $str );
		else
