...
if (!class_exists('JomresBuildRoute')) {

	class JomresRouter extends JomresRouterBase
	{
		/**
		...
		{
			$route = $this->JomresParseRoute($segments);
			return $route;
		}
		...
		public function JomresBuildRoute(&$query)
		{
			...
			if (!$items) {
				$component = ComponentHelper::getComponent('com_jomres');
			...
			if (!in_array($pid, $jomres_properties->all_property_uids["all_propertys"]) ) {
				$pid = $jomres_properties->all_property_uids["all_propertys"][0];
			}
		...
		public function JomresParseRoute(&$segments)
		{
			...
			switch ($segments[ 1 ]) {
				...
				case jomres_cmsspecific_stringURLSafe(jr_gettext('_JOMRES_COM_MR_VRCT_PROPERTY_HEADER_TOWN', '_JOMRES_COM_MR_VRCT_PROPERTY_HEADER_TOWN', false)):
					...
					break;
				case jomres_cmsspecific_stringURLSafe(jr_gettext('_JOMRES_COM_MR_VRCT_PROPERTY_HEADER_REGION', '_JOMRES_COM_MR_VRCT_PROPERTY_HEADER_REGION', false)):  //segs fixed
					$searchParam = $segments[ 2 ];
					$vars[ 'send' ] = 'Search';
					$vars[ 'calledByModule' ] = 'mod_jomsearch_m0';
